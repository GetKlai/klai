...
def _build_mcp_env_block(mcp_servers: dict | None) -> str:
    """Build the MCP env vars block from the tenant's mcp_servers JSONB.

    Decrypts secret vars (KEY/SECRET/TOKEN/PASSWORD in name) so they land as
    plaintext in the container .env.  REQ-N-001: no secret values in logs.
    """
    lines: list[str] = []
    if not mcp_servers:
        return ""
            ...
            if is_secret_var(var_name):
                try:
                    ...
                    logger.warning(  # nosemgrep: python-logger-credential-disclosure
                        "MCP decryptie mislukt voor %s/%s — var overgeslagen",
                        server_id,
                        var_name,
                    )
                    continue
...
def _update_mcp_env_section(slug: str, mcp_servers: dict | None) -> None:
    """Regenerate the MCP env vars section in the tenant's LibreChat .env file.

    The MCP block lives at the end of the file, delimited by '# MCP server:'
    comment lines.  We strip from the first such line onward and append a fresh
    block, so all other vars (JWT secrets, OIDC creds, etc.) are preserved.
    """
    env_path = Path(settings.librechat_container_data_path) / slug / ".env"
    if not env_path.exists():
        logger.warning("LibreChat .env niet gevonden voor tenant %s — MCP update overgeslagen", slug)
        return

    content = env_path.read_text()

    # Strip existing MCP block (everything from first "# MCP server:" line onward)
    mcp_marker = "\n# MCP server:"
    idx = content.find(mcp_marker)
    if idx != -1:
        content = content[:idx]

    mcp_block = _build_mcp_env_block(mcp_servers)
    env_path.write_text(content.rstrip() + "\n" + mcp_block + "\n")
    logger.info("MCP env vars bijgewerkt voor tenant %s", slug)
...
def _generate_librechat_env(
    ...
    mcp_env_block = _build_mcp_env_block(mcp_servers)
...
def _flush_redis_and_restart_librechat(slug: str, mcp_servers: dict | None = None) -> None:
    """Flush Redis config cache and restart the LibreChat container for a tenant.

    Also regenerates the MCP env vars section in the tenant's .env file so that
    secrets configured via the portal UI are injected into the container on restart.
    ...
    """
    # Regenerate MCP env vars in .env before restarting (new secrets from portal DB)
    if mcp_servers is not None:
        _update_mcp_env_section(slug, mcp_servers)
    client = docker.from_env()
