...
Moved from ``deploy/litellm/klai_answer_epistemics.py`` (one-chat-pipeline
plan, slice 3) alongside ``app.services.pasted_correspondence``: this
measures/strips the four-section epistemic contract the model produces when
:data:`app.services.pasted_correspondence.PASTED_CORRESPONDENCE_SCOPE` is in
the prompt. It does not change or enforce the answer itself — generation
callers (slice 2/5) invoke ``strip_answer_contract_markers`` /
``pop_answer_contract_stream_text`` before the markers reach the user.
"""
...
_ANSWER_CONTRACT_PARTIAL_MARKER_RE = re.compile(r"(\[\[KLAI_[A-Z_]*(?:\]?)?)$", re.IGNORECASE)
_ANSWER_CONTRACT_PREFIX = "[[KLAI_"
_ANSWER_CONTRACT_MARKERS_BY_SUFFIX = {
    marker[len("[[KLAI_CORRESPONDENCE_") : -2]: marker for marker in ANSWER_CONTRACT_MARKERS
}
_KNOWN_SECTION_MARKER_RE = re.compile(
    r"\[\[KLAI_[A-Z_]*(" + "|".join(re.escape(suffix) for suffix in _ANSWER_CONTRACT_MARKERS_BY_SUFFIX) + r")\]\]",
    re.IGNORECASE,
)
...
def inspect_answer_epistemics(
    ...
    *,
    ...
    latest_turn_detected = (
        correspondence_detected if latest_turn_correspondence_detected is None else latest_turn_correspondence_detected
    )
    correspondence_text = (
        extract_pasted_correspondence_text(user_turn_text, assume_detected=True) if latest_turn_detected else ""
    )
    correspondence_tokens = extract_salient_query_tokens(correspondence_text)
    evidence_tokens = _evidence_tokens(evidence_chunks)
    measured_answer = strip_answer_contract_markers(answer) if correspondence_detected else answer
    ...
    if correspondence_detected:
        normalized_answer, normalized_marker_count = _normalize_known_section_markers(answer)
...
def _verify_answer_contract(answer: str, evidence_chunks: list[dict]) -> dict[str, Any]:
    ...
    missing_sections = [index for index, position in enumerate(positions, start=1) if position < 0]
    ...
    if evidence_chunks and positions[1] >= 0:
        ...
        valid_ids = {item.evidence_id.upper() for item in evidence_chunks_from_chunks(evidence_chunks)}
        cited_ids = {
            token.upper()
...
def strip_answer_contract_markers(answer: str) -> str:
    ...
    return _strip_trailing_contract_marker_fragment(_ANSWER_CONTRACT_MARKER_RE.sub("", answer))
...
def pop_answer_contract_stream_text(buffer: str, *, final: bool) -> tuple[str, str]:
    ...
    for prefix_length in range(max_prefix, 0, -1):
        if cleaned.casefold().endswith(_ANSWER_CONTRACT_PREFIX[:prefix_length].casefold()):
