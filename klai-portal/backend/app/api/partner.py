...
async def chat_completions(  # noqa: C901
    ...
    # 3a. PDF attachment normalisation (one-chat-pipeline slice 3, moved from
    # the LiteLLM hook's klai_chat_attachments.py). Runs at the entry of the
    # knowledge path, on every request that carries a file part in its latest
    # user turn, regardless of surface — a no-op otherwise. The language
    # decision is taken on the UNMODIFIED messages, same reason as the hook:
    # a Dutch PDF must never overrule an English question.
