...
async def password_reset(body: PasswordResetRequest) -> None:
    ...
    try:
        ...
        logger.exception("send_password_reset failed status=%s", exc.response.status_code)
        return  # fail silently
...
async def password_set(body: PasswordSetRequest) -> None:
    ...
    try:
        ...
        logger.exception("set_password_with_code failed status=%s", exc.response.status_code)
