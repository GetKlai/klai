...
Single source of truth for "which products does a given (role, account type,
...
Three-concept model (Linear / Notion / Slack / GitHub / Auth0):
    account features   = user account type features OR platform-unlocked features
...
# @MX:NOTE: Legacy plan baseline, kept for plan settings/tests while
# portal_orgs.plan remains in the schema. Runtime product entitlement uses
# ACCOUNT_TYPE_PRODUCTS below, keyed by portal_users.seat_type.
...
# @MX:ANCHOR fan_in=2+ -- product baseline by per-user account type.
# Runtime effective_products MUST use this mapping, not portal_orgs.plan.
# The account-type strings are the persisted values of PortalUser.seat_type.
ACCOUNT_TYPE_PRODUCTS: dict[str, frozenset[str]] = {
    "chat": frozenset({"chat", "knowledge"}),
    "knowledge": frozenset({"chat", "knowledge"}),
}
...
def derive_user_products(role: str, account_type: str, platform_unlocked_features: list[str]) -> set[str]:
    """Return the set of products a user with this combination has.

    account_products     granted by the user's account type / seat type
                         (modulo profile floor).
    unlocked_products    granted iff (a) the feature is in
                         ``platform_unlocked_features`` AND (b) the feature has
                         a ``FEATURE_MIN_PROFILE`` entry (i.e. is a user-facing
    ...
    Unknown account_type -> empty account_products. Unknown role -> rank -1,
    fails every FEATURE_MIN_PROFILE check, so user effectively has nothing.
    Both are safe fallbacks that deny rather than over-grant.
    """
    account_products = set(ACCOUNT_TYPE_PRODUCTS.get(account_type, frozenset()))
    ...
    # Account products still need to clear FEATURE_MIN_PROFILE — chat and
    # knowledge default to "personal" so this is a no-op for the current
    # account-type list, but the symmetry matters if a future tier adds a
    # feature with a higher floor.
    account_products = {
        f for f in account_products if caller_rank >= PROFILE_RANK.get(FEATURE_MIN_PROFILE.get(f, "personal"), -1)
    }

    return account_products | unlocked_products
