...
These tests build a synthetic ``UserPermissions`` via ``make_perms`` and pass
it directly into the endpoint signatures. DB + Redis are mocked, so we don't
need Postgres. Full integration-style RLS + migration tests live alongside
...
def _portal_user_with_lc(lc_uid: str = "lc-1") -> MagicMock:
    """Mock PortalUser carrying a librechat_user_id."""
    pu = MagicMock()
    pu.librechat_user_id = lc_uid
    return pu
...
async def test_post_scope_org_as_non_admin_returns_403_nl(monkeypatch):
    ...
    perms = make_perms(role="company", user_id="user-1", org_id=42)
...
async def test_post_scope_personal_as_non_admin_allowed(monkeypatch):
    ...
    monkeypatch.setattr(
        app_templates,
        "_load_portal_user_or_none",
        AsyncMock(return_value=_portal_user_with_lc("lc-1")),
    )
...
async def test_post_scope_org_as_admin_triggers_org_wide_invalidate(monkeypatch):
    ...
    perms = make_perms(role="admin", user_id="user-1", org_id=42)
    body = app_templates.TemplateCreate(name="X", prompt_text="y", scope="org")
...
async def test_patch_by_non_owner_non_admin_returns_403(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await app_templates.update_template(slug="test", body=body, perms=perms, db=MagicMock())
...
async def test_patch_happy_path_flushes_and_refreshes_before_commit(monkeypatch):
    ...
    out = await app_templates.update_template(slug="klantenservice", body=body, perms=perms, db=db)
...
async def test_get_personal_hides_from_non_owner_non_admin(monkeypatch):
    ...
    perms = make_perms(role="company", user_id="user-1", org_id=42)
    with pytest.raises(HTTPException) as exc:
        await app_templates.get_template(slug="test", perms=perms, db=MagicMock())
...
async def test_get_personal_visible_to_admin(monkeypatch):
    ...
    perms = make_perms(role="admin", user_id="admin-1", org_id=42)
    out = await app_templates.get_template(slug="test", perms=perms, db=MagicMock())
