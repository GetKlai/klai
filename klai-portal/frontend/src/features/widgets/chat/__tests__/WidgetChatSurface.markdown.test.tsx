/**
 * Tests for assistant markdown rendering and the user-message XSS boundary.
 *
 * Assistant replies are rendered through snarkdown + DOMPurify (the same
 * approach klai-widget/src/components/MessageList.tsx uses) so bold,
 * italic, and lists show as formatted HTML instead of literal `**`/`*`/`-`
 * characters. User messages never go through that pipeline — they are the
 * visitor's own text and must always render as plain, inert text.
 */
