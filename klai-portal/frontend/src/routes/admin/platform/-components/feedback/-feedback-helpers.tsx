...
export function buildFeedbackDebugInstructions(
  ...
  return [
    ...
    evidence,
    '',
    'Root-cause stopgate (do this before writing code):',
    '1. Do not start from a fix direction. First write down at least 3 competing explanations for the symptom, then look for evidence that confirms or rules each one out.',
    '2. For the fix you are about to make, answer explicitly: which component produced the wrong user-visible behavior, why this is the right layer to change, what production fact would make this fix invalid, and whether this fix would have affected the reported incident.',
    '3. If the proposed fix changes routing, model choice, output budget, retries, caching, or a heuristic based on wording/shape of the report, first prove from production data that the incident was caused by that mechanism. If you cannot prove it, reject that direction.',
    '4. Treat the customer text as symptom evidence only. Do not match on literal phrasing, step numbers, template length, locale, or reporter wording unless production/runtime data proves that is the broken contract.',
    '',
    'Layering check:',
    '- Fix the layer that creates the visible defect: UI only for display bugs, template construction only for malformed template input, retrieval only for wrong evidence selection, streaming/rendering only for emitted-content corruption, and routing/model selection only for proven model-selection or budget failures.',
    '',
