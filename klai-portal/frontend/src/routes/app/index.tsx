...
function ChatHome() {
  ...
  useEffect(() => { if (token) void runHealthCheck(); return clearStuckTimer }, [token, runHealthCheck, clearStuckTimer])
...
function ChatConfigBar({ token }: { token: string | undefined }) {
  const auth = useAuth()
  ...
  const mutation = useMutation({
    mutationFn: async (patch: Partial<Omit<KBPref, 'kb_pref_version'>>) =>
      apiFetch<KBPref>('/api/app/account/kb-preference', token, { method: 'PATCH', body: JSON.stringify(patch) }),
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 pl-4 pr-4 h-[66px] border-b border-[var(--color-sidebar-border)]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
