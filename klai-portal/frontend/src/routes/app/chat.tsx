...
function ChatPage() {
  ...
  // When the klai_sso cookie is stale (Zitadel session expired but portal refresh
  // tokens still work), the login page detects it's in an iframe and sends this
  // message instead of showing a useless login form.
  useEffect(() => {
  ...
  const handleRetry = useCallback(() => {
    // Force a full portal re-auth so the klai_sso cookie gets refreshed.
    // signinRedirect() starts a new OIDC flow → Zitadel → portal login page,
    // which sets a fresh klai_sso cookie. After completion, the chat page
