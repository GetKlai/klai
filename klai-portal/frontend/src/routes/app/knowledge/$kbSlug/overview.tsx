...
function OverviewTab() {
  ...
  const { data: connectors } = useQuery<ConnectorSummary[]>({
    ...
    queryFn: async () =>
      apiFetch<ConnectorSummary[]>(`/api/app/knowledge-bases/${kbSlug}/connectors/`),
    enabled: !!kb,
  })
  ...
  const { data: filesData } = useQuery<PersonalItemsResponse>({
    ...
    queryFn: async () =>
      apiFetch<PersonalItemsResponse>('/api/knowledge/personal/items'),
    enabled: isDefaultPersonal,
  })
  ...
  const deleteKbMutation = useMutation({
    mutationFn: async () =>
      apiFetch(`/api/app/knowledge-bases/${kbSlug}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  const syncAllMutation = useMutation({
    mutationFn: async (connectorIds: string[]) => {
      await Promise.all(
        connectorIds.map((id) =>
          apiFetch(`/api/app/knowledge-bases/${kbSlug}/connectors/${id}/sync`, {
            method: 'POST',
          }),
        ),
      )
  ...
  const syncConnectorMutation = useMutation({
    mutationFn: async (connectorId: string) =>
      apiFetch(`/api/app/knowledge-bases/${kbSlug}/connectors/${connectorId}/sync`, {
        method: 'POST',
      }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['kb-connectors-portal', kbSlug] })
  ...
  const deleteConnectorMutation = useMutation({
    mutationFn: async (connectorId: string) =>
      apiFetch(`/api/app/knowledge-bases/${kbSlug}/connectors/${connectorId}`, {
        method: 'DELETE',
      }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['kb-connectors-portal', kbSlug] })
  ...
  const deleteFileMutation = useMutation({
    mutationFn: async (artifactId: string) =>
      apiFetch(`/api/knowledge/personal/items/${artifactId}`, { method: 'DELETE' }),
    onSuccess: () => {
