...
function CollectionRow({
  ...
  const myUserId: string | undefined = currentUser?.user_id
  ...
  const { data: connectors, isLoading: connectorsLoading } = useQuery<ConnectorSummary[]>({
    ...
    queryFn: () => apiFetch<ConnectorSummary[]>(`/api/app/knowledge-bases/${kb.slug}/connectors/`),
    enabled: expanded,
  })
  ...
  const { data: personalFilesData } = useQuery<{ items: { id: string; path: string }[] }>({
    ...
    enabled: expanded && isDefaultPersonalRow,
  })
  const personalFiles = personalFilesData?.items ?? []

  const deleteKbMutation = useMutation({
    mutationFn: async () => apiFetch(`/api/app/knowledge-bases/${kb.slug}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  const syncConnectorMutation = useMutation({
    mutationFn: async (connectorId: string) =>
      apiFetch(`/api/app/knowledge-bases/${kb.slug}/connectors/${connectorId}/sync`, { method: 'POST' }),
    onSuccess: () => {
  ...
  const deleteConnectorMutation = useMutation({
    mutationFn: async (connectorId: string) =>
      apiFetch(`/api/app/knowledge-bases/${kb.slug}/connectors/${connectorId}`, { method: 'DELETE' }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['kb-connectors-portal', kb.slug] })
      void queryClient.invalidateQueries({ queryKey: ['app-knowledge-bases-stats-summary'] })
