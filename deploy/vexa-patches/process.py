...
async def start_bot_container(
    ...
    bot_config = {
        ...
        "automaticLeave": {
            ...
            "noOneJoinedTimeout": 30000,    # 30 seconds
            "everyoneLeftTimeout": 5000      # 5 seconds — stop immediately when meeting ends
        },
        "botManagerCallbackUrl": f"{BOT_CALLBACK_BASE_URL}/bots/internal/callback/exited"
    }
    # Recording config -- mirrors Docker orchestrator (orchestrator_utils.py)
    bot_config["recordingEnabled"] = (
        bool(recording_enabled) if recording_enabled is not None
        else os.getenv("RECORDING_ENABLED", "false").lower() == "true"
    )
    bot_config["captureModes"] = os.getenv("CAPTURE_MODES", "audio").split(",")
    bot_config["recordingUploadUrl"] = f"{BOT_CALLBACK_BASE_URL}/internal/recordings/upload"
