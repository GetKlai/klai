 ...
 */
class ScreenContentService {
    constructor(page, defaultAvatarUrl) {
        ...
        this._defaultAvatarDataUri = null;
        ...
        if (defaultAvatarUrl) {
            ...
            (0, utils_1.log)(`[ScreenContent] Custom default avatar URL set from config: ${defaultAvatarUrl.substring(0, 80)}...`);
        }
        ...
        try {
            ...
            for (const p of possiblePaths) {
                if (fs.existsSync(p)) {
                    ...
                    (0, utils_1.log)(`[ScreenContent] Default avatar loaded from ${p} (${buf.length} bytes)`);
                    return;
                }
            }
            ...
            (0, utils_1.log)('[ScreenContent] Default avatar: using embedded Vexa logo (no logo PNG found on disk)');
        }
        catch (err) {
            (0, utils_1.log)(`[ScreenContent] Failed to load default avatar: ${err.message}`);
            this._defaultAvatarDataUri = EMBEDDED_LOGODARK_DATA_URI;
        }
    }
    /**
    ...
    async initialize() {
        ...
        const status = await this.page.evaluate(() => {
            const canvas = window.__vexa_canvas;
            const ctx = window.__vexa_canvas_ctx;
            const stream = window.__vexa_canvas_stream;
            return {
                hasCanvas: !!canvas,
                hasCtx: !!ctx,
                hasStream: !!stream,
        ...
        if (!status.hasCanvas || !status.hasCtx || !status.hasStream) {
            ...
            (0, utils_1.log)('[ScreenContent] Init script canvas not found, creating fallback canvas...');
            await this.page.evaluate(() => {
                if (window.__vexa_canvas)
                ...
                window.__vexa_canvas_stream = stream;
            });
        }
        this._initialized = true;
        (0, utils_1.log)(`[ScreenContent] Canvas virtual camera initialized (initScript canvas: ${status.hasCanvas}, tracks: ${status.videoTracks})`);
        ...
        if (avatarUri) {
            ...
            (0, utils_1.log)('[ScreenContent] Default avatar drawn on canvas');
        }
     ...
     */
    async _startFramePump() {
        await this.page.evaluate(() => {
            // Don't start twice
            if (window.__vexa_frame_pump_active)
                return;
            window.__vexa_frame_pump_active = true;
            const canvas = window.__vexa_canvas;
            const ctx = window.__vexa_canvas_ctx;
            if (!canvas || !ctx)
                return;
        ...
        (0, utils_1.log)('[ScreenContent] Frame pump started');
    }
    /**
     ...
     */
    async tryTeamsVideoOptionsFallback() {
        ...
        try {
            ...
            (0, utils_1.log)(`[ScreenContent] Opened video options${label ? ` ("${label}")` : ''}`);
            await this.page.waitForTimeout(700);
        }
        catch (err) {
            (0, utils_1.log)(`[ScreenContent] Failed to open video options: ${err.message}`);
            return false;
        }
        ...
        try {
            ...
            if (vexaVisible) {
                ...
                deviceSelected = true;
                (0, utils_1.log)('[ScreenContent] Selected "Vexa Virtual Camera" in video options');
            }
        }
        catch { }
        if (!deviceSelected) {
            const fallbackCameraLabel = await this.page.evaluate(() => {
                ...
                const candidates = Array.from(document.querySelectorAll('[role="menuitemradio"], [role="option"], button, [data-tid], [aria-label]'));
                for (const el of candidates) {
                    if (!isVisible(el))
                        continue;
                    const label = normalize(el.innerText || el.getAttribute('aria-label'));
                    if (!label)
                        continue;
                    const lower = label.toLowerCase();
                    const isCameraDeviceCandidate = lower.includes('camera') &&
            ...
            if (fallbackCameraLabel) {
                deviceSelected = true;
                (0, utils_1.log)(`[ScreenContent] Selected fallback camera option: "${fallbackCameraLabel}"`);
            }
            else {
                (0, utils_1.log)('[ScreenContent] No selectable camera device found in video options');
            }
        }
        ...
        if (turnOffVisible) {
            (0, utils_1.log)('[ScreenContent] Video options fallback succeeded; camera appears ON');
            return true;
        }
        (0, utils_1.log)('[ScreenContent] Video options fallback did not expose a camera-ON state');
    ...
    async enableCamera() {
        ...
        const toolbarButtons = await this.page.evaluate(() => {
            ...
            return buttons
                ...
                .filter(b => b.ariaLabel.toLowerCase().includes('camera') ||
        ...
        (0, utils_1.log)(`[ScreenContent] Camera-related buttons: ${JSON.stringify(toolbarButtons)}`);
        ...
        try {
            ...
            (0, utils_1.log)(`[ScreenContent] Found camera/video button: "${label}", clicking...`);
            await turnOnCameraBtn.click({ force: true });
            (0, utils_1.log)('[ScreenContent] Clicked camera/video button — getUserMedia patch will provide canvas stream');
            // Wait for camera to initialize and getUserMedia to be called
            await this.page.waitForTimeout(3000);
        }
        catch {
            (0, utils_1.log)('[ScreenContent] Camera/video button not found — trying "Turn off" check (maybe already on)');
            ...
            try {
                ...
                (0, utils_1.log)('[ScreenContent] Camera/video is already ON (found "Turn off" button)');
            }
            catch {
                (0, utils_1.log)('[ScreenContent] Neither camera/video on nor off button found — trying video options fallback');
                const fallbackEnabled = await this.tryTeamsVideoOptionsFallback();
                if (!fallbackEnabled) {
                    (0, utils_1.log)('[ScreenContent] Video options fallback unavailable or unsuccessful');
                }
            }
        }
        ...
        const diagnostic = await this.page.evaluate(() => {
            ...
            const canvasStream = window.__vexa_canvas_stream;
            ...
            for (let i = 0; i < pcs.length; i++) {
                ...
                try {
                    for (const t of pc.getTransceivers()) {
                        if (t.sender && (t.receiver?.track?.kind === 'video' ||
            ...
            return {
                ...
                gumCallCount: window.__vexa_gum_call_count || 0,
                gumVideoIntercepted: window.__vexa_gum_video_intercepted || 0,
                addTrackIntercepted: window.__vexa_addtrack_intercepted || 0,
            };
        });
        (0, utils_1.log)(`[ScreenContent] Camera diagnostic: ${JSON.stringify(diagnostic)}`);
        ...
        (0, utils_1.log)('[ScreenContent] Attempting replaceTrack to inject canvas stream into WebRTC...');
        const replaceResult = await this.page.evaluate(async () => {
            ...
            window.__vexa_canvas_stream = freshStream;
            const canvasTrack = freshStream.getVideoTracks()[0];
            if (!canvasTrack)
                return { success: false, reason: 'failed to get canvas track from fresh stream' };
            ...
            for (let i = 0; i < pcs.length; i++) {
                ...
                try {
                    ...
                    for (const t of transceivers) {
                        ...
                        const isSendVideo = (t.direction === 'sendonly' || t.direction === 'sendrecv') &&
                            (t.sender !== null) &&
        ...
        (0, utils_1.log)(`[ScreenContent] replaceTrack result: ${JSON.stringify(replaceResult)}`);
        ...
        (0, utils_1.log)(`[ScreenContent] SDP diagnostic: ${JSON.stringify(sdpDiag)}`);
    }
    /**
     ...
     */
    async toggleCameraForRenegotiation() {
        (0, utils_1.log)('[ScreenContent] Attempting camera toggle (off→on) to force SDP renegotiation...');
        ...
        try {
            ...
            try {
                ...
                (0, utils_1.log)(`[ScreenContent] Toggle: clicking OFF button ("${label}")...`);
                ...
                (0, utils_1.log)('[ScreenContent] Toggle: no "turn off" button found — camera may already be off');
            }
            ...
            try {
                ...
                (0, utils_1.log)(`[ScreenContent] Toggle: clicking ON button ("${label}")...`);
                await turnOnBtn.click({ force: true });
                // Wait for getUserMedia, replaceTrack, and SDP renegotiation
                await this.page.waitForTimeout(3000);
                (0, utils_1.log)('[ScreenContent] Toggle: camera toggled on — SDP renegotiation should be in progress');
                // Now run replaceTrack to ensure our canvas stream is the active video source
                await this.enableCamera();
                return true;
            }
            catch {
                (0, utils_1.log)('[ScreenContent] Toggle: no "turn on" button found — trying video options fallback');
                const fallbackEnabled = await this.tryTeamsVideoOptionsFallback();
                if (fallbackEnabled) {
                    await this.enableCamera();
                    return true;
                }
                (0, utils_1.log)('[ScreenContent] Toggle: video options fallback failed');
                return false;
            }
        }
        catch (err) {
            (0, utils_1.log)(`[ScreenContent] Toggle failed: ${err.message}`);
            return false;
        }
    }
    /**
    ...
    async showImage(imageSource) {
        ...
        const success = await this.page.evaluate(async (imgSrc) => {
            const canvas = window.__vexa_canvas;
            const ctx = window.__vexa_canvas_ctx;
            if (!canvas || !ctx)
                return false;
        ...
        if (success) {
            ...
            (0, utils_1.log)(`[ScreenContent] Showing image on virtual camera: ${imageSource.substring(0, 80)}...`);
        }
        else {
            (0, utils_1.log)(`[ScreenContent] Failed to load image: ${imageSource.substring(0, 80)}...`);
        }
     ...
     */
    async showText(text, fontSize = 48) {
        if (!this._initialized)
            await this.initialize();
        await this.page.evaluate(({ text, fontSize }) => {
            const canvas = window.__vexa_canvas;
            const ctx = window.__vexa_canvas_ctx;
            if (!canvas || !ctx)
                return;
        ...
        this._currentUrl = null;
        (0, utils_1.log)(`[ScreenContent] Showing text on virtual camera: "${text.substring(0, 50)}..."`);
        await this.enableCamera();
    }
    /**
    ...
    async clearScreen() {
        ...
        this._currentContentType = null;
        this._currentUrl = null;
        (0, utils_1.log)('[ScreenContent] Screen cleared (showing default avatar)');
    }
    /**
     * Set a custom avatar image (replaces the default Vexa logo).
     * @param imageSource URL or base64 data URI of the avatar image
     */
    async setAvatar(imageSource) {
        let src = imageSource;
        if (!imageSource.startsWith('http') && !imageSource.startsWith('data:')) {
            src = `data:image/png;base64,${imageSource}`;
        }
        this._customAvatarDataUri = src;
        (0, utils_1.log)(`[ScreenContent] Custom avatar set: ${src.substring(0, 60)}...`);
    ...
    async resetAvatar() {
        this._customAvatarDataUri = null;
        (0, utils_1.log)('[ScreenContent] Avatar reset to default');
        // If currently showing avatar (no active content), refresh the display
        if (!this._currentContentType && this._initialized) {
            const avatarUri = this._getAvatarDataUri();
            if (avatarUri) {
     ...
     */
    async _drawAvatarOnCanvas(avatarUri) {
    ...
    async close() {
        ...
        this._currentUrl = null;
        this._initialized = false;
        (0, utils_1.log)('[ScreenContent] Content service closed');
    }
    /**
...
exports.ScreenContentService = ScreenContentService;
/**
 ...
 */
function getVirtualCameraInitScript() {
    return `
    (() => {
 ...
 */
function getVideoBlockInitScript() {
    return `
    (() => {
...
//# sourceMappingURL=screen-content.js.map