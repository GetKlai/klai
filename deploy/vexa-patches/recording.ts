      ...
      await new Promise<void>((resolve, reject) => {
        try {
          ...
          (async () => {
            ...
            // Participant counting: data-participant-id first, text scan as fallback.
            // Fixes upstream bug where text scan counts UI labels as participants.
            // See: https://github.com/Vexa-ai/vexa/issues/190
            (window as any).logBot("Initializing participant counting (data-participant-id + text scan fallback)...");

            const extractParticipantsFromMain = (_botName: string | undefined): string[] => {
              // Primary: attribute-based detection (reliable, same selector used for admission check)
              const byAttribute = document.querySelectorAll('[data-participant-id]');
              if (byAttribute.length > 0) {
                return Array.from(byAttribute).map(
                  (el: Element, i: number) => el.getAttribute('data-participant-id') || `p-${i}`
                );
              }
              // Fallback: text scan (original method, used if data-participant-id unavailable)
              const participants: string[] = [];
              const mainElement = document.querySelector('main');
              if (mainElement) {
                mainElement.querySelectorAll('*').forEach((el: Element) => {
                  const element = el as HTMLElement;
                  const text = (element.textContent || '').trim();
                  if (text && element.children.length === 0 && text.length > 1 && text.length < 50) {
