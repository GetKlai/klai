...
`whisper-server` is the exception to the exception: it is built and pushed by CI like the others, but **nothing pulls it automatically** — it runs on gpu-01, whose compose lives in `GetKlai/klai-infra`, and is rolled out by hand. Its `:latest` is a pointer to the newest build, not a deployment. A green `whisper-server` build does not mean the fix is live. (research-api removed in SPEC-PORTAL-UNIFY-KB-001.) Each CI workflow also tags the build with `:${github.sha}` so rollbacks are possible via explicit SHA pin. These are NOT production `:latest` anti-patterns — they are continuous-deployment rolling tags owned by our own CI pipelines. The self-host compose file expects every referenced `ghcr.io/getklai/*` image to be public and anonymously pullable; `deploy/check-image-pullable.sh` enforces that contract.
