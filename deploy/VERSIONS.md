...
**Exception — internal CI-deployed services:** images under `ghcr.io/getklai/*` (portal-api, retrieval-api, knowledge-ingest, klai-connector, klai-mailer, klai-docs, klai-knowledge-mcp, scribe-api, caddy-hetzner) use `:latest` because GitHub Actions rebuild and re-push on every commit to `main` in their respective repos, AND — this is the part that makes the rolling tag safe — the pull and the recreate happen in one `compose-up.sh <service>` call, so a newer `:latest` never sits on the host waiting to be picked up by somebody else's deploy.

Each CI workflow also tags the build with `:${github.sha}` so rollbacks are possible via explicit SHA pin. These are NOT production `:latest` anti-patterns — they are continuous-deployment rolling tags owned by our own CI pipelines. The self-host compose file expects every referenced `ghcr.io/getklai/*` image to be public and anonymously pullable; `deploy/check-image-pullable.sh` enforces that contract. (research-api removed in SPEC-PORTAL-UNIFY-KB-001.)

`whisper-server` is the exception to the exception, and not for the reason this file claimed until 2026-08-17. It is not deployed anywhere — not automatically, and not by hand. `docs/architecture/scribe-transcription.md` calls it what it is: a public reference image for self-hosters who need a Whisper-compatible speech-to-text backend. Klai production has not run it since SPEC-VEXA-003 replaced it with the Vexa transcription workers, which serve the same host port and the same OpenAI-compatible path, so `WHISPER_SERVER_URL` never pointed at a whisper-server container to begin with. A green build here means the reference image is publishable — nothing more, and nothing was waiting on it.

**Exception — local builds:** `klai/retrieval-api:local` is built on-host from source and not pullable from a registry. Its "version" is tracked by the git SHA recorded in docker-compose.yml comments.

`firecrawl-api` used to be in that list and no longer is. It was built on-host and tagged `ghcr.io/mendableai/firecrawl:latest`, a name nothing could pull, so the running service could not have been rebuilt from this repository if the host had lost the image. Since 2026-09-10 it is pinned to `ghcr.io/firecrawl/firecrawl:2.11.325`, upstream's own published release, so Renovate tracks it like every other image here. Bumping it means running the comparison recorded in the docker-compose.yml comment first: identical harness and env contract, and LibreChat's `POST /v2/scrape` answering with the same shape.

**Exception — Vexa stack (upstream v0.13.1, 2026-09-23):** `vexaai/v012-admin-api`, `vexaai/v012-meeting-api`, `vexaai/v012-runtime` and the bot image `vexaai/vexa-bot` are on `v0.13.1` — pulled directly from Docker Hub (since v0.10.4 upstream publishes pre-built images). The 0.10 line (`admin-api`, `api-gateway`, `meeting-api`, `runtime-api`, `vexa-redis`) was removed in PR #914 after 0.12 was proven end to end; SPEC-VEXA-004 has the migration. Note the tag shape: the 0.10 line published `0.10.6.3.14`, the 0.12 line publishes ONLY the v-prefixed form — plain `0.12.22` is not on Docker Hub. `deploy/check-image-tags.sh` accepts either. The bot image is an env value rather than a compose service, so `compose up` never fetches it and docker-socket-proxy cannot either; it is pulled by hand and checked by `scripts/check-vexa12-deploy-preconditions.sh`. Upgrade cadence: track upstream stable tags; bump for material fixes (chunk-leak, OOM, security, browser/admission fixes). See `https://github.com/Vexa-ai/vexa/releases` for changelog.
...
| `alloy` | `grafana/alloy:v1.19.2` | Log and metric collection. Config format stable on minor bumps. 1.19.2 (2026-09-11) is bug fixes only. This row said 1.18.1 while compose ran 1.19.0, so it had drifted a release behind — the pin in compose is the one to trust. |
...
| `gitea` | `gitea/gitea:1.27.3` | Self-hosted git for klai-docs. Bumped 2026-08-13: 1.27.1 patches CVE-2026-59774 (critical, unauthenticated file read → RCE) + CVE-2026-60004 (critical, RCE via diffpatch hooks); 1.27.0 patches the high SSRF + PAT-scope CVEs. 1.27.0 breaking changes (Actions reusable workflows, CSP script nonce) do not affect us — Gitea is git-hosting only here. Bumped to 1.27.3 on 2026-09-11: a security release with eighteen fixes, among them package token-scope access, attachment path enforcement, same-repository issue access, and fork pull-request trust boundaries. This row said 1.27.1 while compose ran 1.27.2; the compose pin is the one to trust. |
| `crawl4ai` | `ghcr.io/getklai/crawl4ai:0.9.4-local-260925-2225` | Klai-built image from upstream 0.9.4 (deploy/crawl4ai/Dockerfile, built on core-01, never pushed to a registry). History: 0.9.0 patched CVE-2026-57571 + CVE-2026-57572 (both critical, RCE-class) and CVE-2026-57573 (high SSRF on /crawl/stream); we moved to 0.9.2 on 2026-08-13 after adapting the knowledge-ingest client to the 0.9 untrusted-config model (#873 — DOM prep moved into the allowlisted wait_for predicate, js_code fields gone), to 0.9.3 on 2026-09-11, and to 0.9.4 on 2026-09-25 (security release: two SSRF fixes and the dict-wrapper untrusted-gate bypass, GHSA-5w5p-vcv6-mm3f; no request-contract change). 0.9.x enforces bearer auth on /crawl; knowledge-ingest and connector both carry the shared key. Hooks are ENABLED in compose since 2026-09-11: on 0.9.x a hook is a declarative action from a fixed server-authored set, not the exec'd Python that CVE-2026-57572 closed, and leaving the flag at its 0.8.x value silently dropped cookie injection from every authenticated crawl. EXACTLY ONE local patch remains: a request carrying hooks gets its own browser, because upstream attaches hooks to a pooled crawler and never detaches them. The body-visibility patch is gone — 0.9.3 ships `body_visibility_timeout` as a config field (our issue #2129), set per request in knowledge_ingest/crawl4ai_config.py. All three contracts are proven by deploy/tests/crawl4ai-cookie-contract.sh. |
| `presidio` | analyzer `ghcr.io/getklai/presidio-analyzer@sha256:0605d1e9…` (2.2.364-klai.1), anonymizer upstream `ghcr.io/data-privacy-stack/presidio-anonymizer@sha256:e5670138…` (2.2.364) | Digest-pinned, so Renovate cannot follow it; `version-drift.yml` compares against the newest upstream tag. Moved from 2.2.362 on 2026-09-25 for 183 Trivy findings; detection only widened (UK/FR phone regions, wider email pattern, six IBAN countries). The analyzer is rebuilt by presidio-analyzer-image-build.yml on merge and pinned here in a follow-up. |
