...
| `crawl4ai` | `ghcr.io/getklai/crawl4ai:0.9.2-local-260911-0808` | Klai-patched build of upstream 0.9.2 (bumped 2026-08-13): 0.9.0 patches CVE-2026-57571 + CVE-2026-57572 (both critical, RCE-class) and CVE-2026-57573 (high SSRF on /crawl/stream). Klai-patched build of upstream 0.9.2 (landed 2026-08-13, second attempt): 0.9.0 patches CVE-2026-57571 + CVE-2026-57572 (both critical, RCE-class) and CVE-2026-57573 (high SSRF). The knowledge-ingest client was first adapted to the 0.9 untrusted-config model (#873 — DOM prep moved into the allowlisted wait_for predicate; js_code fields gone), verified byte-equal on both server versions before this flip. The local body-visibility patch (upstream PR #2131 still open) applies anchor-guarded on 0.9.2. 0.9.x enforces bearer auth on /crawl; knowledge-ingest + connector both carry the shared key. Hooks are ENABLED in compose since 2026-09-11: on 0.9.x a "hook" is a declarative action from a fixed server-authored set, not the exec'd Python that CVE-2026-57572 closed, and leaving the flag at its 0.8.x value silently dropped cookie injection from every authenticated crawl. The rebuild of the same day adds a second anchor-guarded patch giving a hook-carrying request its own browser instead of a pooled one, because upstream attaches hooks to the crawler and never detaches them. Both are proven by deploy/tests/crawl4ai-cookie-contract.sh. |
