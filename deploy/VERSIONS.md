...
| `crawl4ai` | `ghcr.io/getklai/crawl4ai:0.8.9-local-260806-1448` | Klai-patched build of upstream 0.9.2 (bumped 2026-08-13): 0.9.0 patches CVE-2026-57571 + CVE-2026-57572 (both critical, RCE-class) and CVE-2026-57573 (high SSRF on /crawl/stream). BLOCKED on 0.9.2 (2026-08-13 attempt rolled back): 0.9.x treats every network request as untrusted and hard-rejects the js_code/js_code_before_wait fields our knowledge-ingest client sends, breaking all crawls/previews with HTTP 400. Upgrade path: adapt the client to the 0.9 untrusted-config model (drop js_code, rely on excluded_tags/target_elements), then re-bump — the patched 0.9.2 image ghcr.io/getklai/crawl4ai:0.9.2-local-260813-1211 is already built on core-01, and the CRAWL4AI_API_KEY env plumbing for knowledge-ingest stays in place (0.9.x enforces bearer auth). Hooks stay disabled in compose. |
