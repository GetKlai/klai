...
| `transcription-worker-1`, `transcription-worker-2` | `vexaai/transcription-service:0.10.0-260419-1129` (locally built on gpu-01 — NOT on Docker Hub) | Vexa transcription-service (SPEC-VEXA-003 §3.4). Replaces the legacy custom `whisper-server` 146-line Python script. `faster-whisper` + Silero VAD + hallucination detection + two-tier admission (realtime/deferred) behind Nginx LB. CUDA 12.3.2 + cuDNN 9. Host port `127.0.0.1:8000` retained so `gpu-tunnel.service` and all consumer URLs stay unchanged. Runs on the v0.10.0-base image; bump only when a transcription-specific upstream change warrants a fresh `make build` on gpu-01. |
