"""Statement-level grounding check and repair for the internal chat.
...
tickets — so the same number was not a given. Measured evenly on 80 stored
answers per path on 2026-09-18 it is worse here: 85% state something the
articles do not carry (widget: 75%), and 72% reach the repair threshold
(widget: 65%). Internal answers are three times longer (median 1173 characters
against 349).

Since #1526 and #1530 (both 2026-09-18) the repair therefore runs here too, for
every tenant: there is no per-organisation switch, the only conditions are the
ones in ``_repair_would_be_wrong`` (Strict mode, citable sources, the whole
answer still in hand, nothing resting on what the user pasted). Same words,
same schema and same threshold as the widget, shared through
``klai_chat_prompts``; a wider time budget because the answers are longer.
...
been read: there ``log_answer_grounding`` only measures, on a task that delays
...
been read. There the caller waits for ``repair_answer``, and the user waits with
it: median 4.0 s and 8.3 s in the slowest tenth, measured on seventeen real
internal answers on 2026-09-18, which it changed nine of.
"""
