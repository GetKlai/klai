"""Tests for the vendored flat module ``klai_conversation_language``.

This is the copy the LiteLLM container actually runs: importable as a FLAT
module (no package) with the vendored ``langid`` directory next to it, both
bind-mounted onto PYTHONPATH. The suite body is identical to
``klai-libs/chat-prompts/tests/test_language.py`` — only the import header
differs — so a behaviour change in the vendored file fails here even if the
canonical suite somehow passes.
"""
...
import klai_conversation_language as clm
