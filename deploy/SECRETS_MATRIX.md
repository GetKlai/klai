...
| `LITELLM_GENERAL_CHAT_KEY` | portal-api | Dedicated LiteLLM virtual key for plain OpenAI-compatible `/partner/v1/chat/completions` and `/partner/v1/responses` calls; MUST be distinct from `LITELLM_MASTER_KEY` and must carry its own LiteLLM budget/RPM/TPM limits. |
...
Mistral Pro pool (litellm): `MISTRAL_API_KEY_2` is the key of a second Pro organisation with pay-as-you-go off. It takes over when the Klai organisation is full (Mistral answers 402), and `sync-env` treats it as required.
