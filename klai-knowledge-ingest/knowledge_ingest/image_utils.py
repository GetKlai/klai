...
SPEC-CRAWLER-004 Fase A — ported verbatim from
``klai-connector/app/services/image_utils.py`` (introduced in commit
``28dda391`` as a last-minute fix for Cloudflare srcset debris poisoning
the crawl4ai image list). Lives here so knowledge-ingest's consolidated
crawl pipeline gets the same URL hygiene without depending on the
klai-connector package.
"""
