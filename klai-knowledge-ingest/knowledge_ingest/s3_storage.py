...
SPEC-CRAWLER-004 Fase A — ported from
``klai-connector/app/services/s3_storage.py``. The store wraps the
synchronous minio client with :func:`asyncio.to_thread` so the
knowledge-ingest event loop (Procrastinate workers, FastAPI endpoints)
stays non-blocking. Images are uploaded authenticated over the S3 API
(Garage on :3900) and served anonymously via Garage website mode
through a Caddy reverse proxy at ``/kb-images/{object_key}``.

Content-addressed keys (SHA-256 of bytes) give free deduplication across
tenants' own KBs and match the bit-for-bit layout the klai-connector
pipeline already writes to Garage, so a consolidated pipeline keeps
existing images reachable under the same public URL.
"""
...
# @MX:REASON: Every crawl page calls upload_image(); changing the key format
#   or URL prefix breaks every previously uploaded image's public URL.
# @MX:SPEC: SPEC-KB-IMAGE-001, SPEC-CRAWLER-004
class ImageStore:
