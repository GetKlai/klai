"""
MCP Bridge for CodeIndex

Starts the CodeIndex MCP server as a subprocess and provides a Python interface
...
class MCPBridge:
    """
    Manages a CodeIndex MCP server subprocess and proxies tool calls to it.
    ...
    def start(self) -> bool:
        """Start the CodeIndex MCP server subprocess."""
        if self._started:
            return True

        try:
            # Find codeindex binary
            codeindex_bin = self._find_codeindex()
            if not codeindex_bin:
                logger.error("CodeIndex not found. Install with: npm install -g codeindex")
                return False
            ...
            init_result = self._send_request("initialize", {
                ...
                "capabilities": {},
                "clientInfo": {"name": "codeindex-eval", "version": "0.1.0"},
            })
    ...
    def call_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any] | None:
        """
        Call a CodeIndex MCP tool and return the result.
    ...
    def _find_codeindex(self) -> str | None:
        """Find the codeindex CLI binary."""
        # Check if npx is available (preferred - uses local install)
        for cmd in ["npx"]:
            try:
                result = subprocess.run(
                    [cmd, "codeindex", "--version"],
                    capture_output=True, text=True, timeout=15,
                    cwd=self.repo_path,
                )
                if result.returncode == 0:
                    return cmd  # Will use "npx codeindex mcp"
            except Exception:
                continue
...
class MCPToolCLI:
    ...
    def run(self, tool_name: str, args_json: str = "{}") -> int:
        ...
        if not self.bridge.start():
            print("ERROR: Failed to start CodeIndex MCP bridge", file=sys.stderr)
            return 1
