#!/usr/bin/env python3
"""
Results Analyzer for CodeIndex SWE-bench Evaluation
...
def compute_metrics(run_data: dict) -> dict:
    ...
    for instance_id, traj in trajectories.items():
        ...
        gn = info.get("codeindex", {}).get("metrics", {})
        if gn:
...
def codeindex_usage(
    ...
    """Analyze CodeIndex tool usage patterns across all runs."""
    results_path = Path(results_dir)
    runs = load_run_results(results_path)

    console.print("\n[bold]CodeIndex Tool Usage Analysis[/bold]\n")
    ...
    for run_id, run_data in sorted(runs.items()):
        ...
        for traj in run_data.get("trajectories", {}).values():
            gn = traj.get("info", {}).get("codeindex", {}).get("metrics", {})
