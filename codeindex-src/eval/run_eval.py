#!/usr/bin/env python3
"""
CodeIndex SWE-bench Evaluation Runner

Main entry point for running SWE-bench evaluations with and without CodeIndex.
Supports running a single configuration or a full matrix of models x modes.

Usage:
    # Single run (default: native_augment mode — CodeIndex tools + grep enrichment)
...
logger = logging.getLogger("codeindex_eval")
console = Console()
...
def process_instance(
    ...
    try:
        ...
        if env_class_name == "eval.environments.codeindex_docker.CodeIndexDockerEnvironment":
            from environments.codeindex_docker import CodeIndexDockerEnvironment
            env_config["image"] = get_swebench_docker_image(instance)
            env = CodeIndexDockerEnvironment(**env_config)
        else:
        ...
        agent_class_name = agent_config.pop("agent_class", "eval.agents.codeindex_agent.CodeIndexAgent")

        from agents.codeindex_agent import CodeIndexAgent
        traj_path = instance_dir / f"{instance_id}.traj.json"
        agent_config["output_path"] = traj_path
        agent = CodeIndexAgent(model, env, **agent_config)
        ...
        result["codeindex_metrics"] = agent.codeindex_metrics.to_dict()
...
def list_configs():
    ...
    for name in AVAILABLE_MODES:
        ...
        gn_mode = config.get("agent", {}).get("codeindex_mode", "baseline")
        console.print(f"  {name:<20} codeindex_mode={gn_mode}")
...
def _print_summary(results: list[dict], model: str, mode: str):
    ...
    # CodeIndex-specific metrics
    gn_tool_calls = sum(
        r.get("codeindex_metrics", {}).get("total_tool_calls", 0) for r in results
    )
    gn_augment_hits = sum(
        r.get("codeindex_metrics", {}).get("augmentation_hits", 0) for r in results
    )
    if gn_tool_calls > 0:
        table.add_row("CodeIndex Tool Calls", str(gn_tool_calls))
...
def _print_matrix_summary(all_results: dict[str, list[dict]]):
    ...
    for run_id, results in sorted(all_results.items()):
        ...
        gn_calls = sum(r.get("codeindex_metrics", {}).get("total_tool_calls", 0) for r in results)
