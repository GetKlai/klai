...
Evaluate whether CodeIndex code intelligence improves AI agent performance on real software engineering tasks. Runs SWE-bench instances across multiple models and compares baseline (no graph) vs CodeIndex-enhanced configurations.
...
| `native` | Baseline + explicit CodeIndex tools via eval-server (~100ms) |
| `native_augment` | Native tools + grep results automatically enriched with graph context (**recommended**) |

> **Recommended**: Use `native_augment` mode. It mirrors the Claude Code model — the agent gets both explicit CodeIndex tools (fast bash commands) AND automatic enrichment of grep results with callers, callees, and execution flows. The agent decides when to use explicit tools vs rely on enriched search output.
...
# Baseline comparison (no CodeIndex)
...
# CodeIndex tool usage analysis
python -m analysis.analyze_results codeindex-usage results/
...
eval/
  ...
  agents/
    codeindex_agent.py      # CodeIndexAgent: extends DefaultAgent with augmentation + metrics
  environments/
    codeindex_docker.py     # Docker env with CodeIndex + eval-server + standalone tool scripts
  bridge/
    codeindex_tools.sh      # Bash wrappers (legacy — now standalone scripts are installed directly)
  ...
  prompts/
    ...
    system_native.jinja            # System: + CodeIndex tool reference
    instance_native.jinja          # Instance: + CodeIndex debugging workflow
    system_native_augment.jinja    # System: + CodeIndex tools + grep enrichment docs
    instance_native_augment.jinja  # Instance: + CodeIndex workflow + risk assessment
  configs/
...
2. **CodeIndex setup**: Node.js + codeindex installed, `codeindex analyze` runs (or restores from cache)
3. **Eval-server starts**: `codeindex eval-server` daemon (persistent HTTP server, keeps KuzuDB warm)
4. **Standalone tool scripts installed** in `/usr/local/bin/` — works with `subprocess.run` (no `.bashrc` needed)
5. Agent runs with the configured model + system prompt + CodeIndex tools
6. Agent's patch is extracted as a git diff
7. Metrics collected: cost, tokens, tool calls, CodeIndex usage, augmentation stats

### Tool architecture

```
Agent → bash command → /usr/local/bin/codeindex-query
  → curl localhost:4848/tool/query     (fast path: eval-server, ~100ms)
  → npx codeindex query                 (fallback: cold CLI, ~5-10s)
```
...
SWE-bench repos repeat (Django has 200+ instances at different commits). The harness caches CodeIndex indexes per `(repo, commit)` hash in `~/.codeindex-eval-cache/` to avoid redundant re-indexing.

### Grep augmentation (native_augment mode)

When the agent runs `grep` or `rg`, the observation is post-processed: the agent class calls `codeindex-augment` on the search pattern and appends `[CodeIndex]` annotations showing callers, callees, and execution flows for matched symbols. This mirrors the Claude Code / Cursor hook integration.
...
| GN Tool Calls | How many CodeIndex tools the agent used |
