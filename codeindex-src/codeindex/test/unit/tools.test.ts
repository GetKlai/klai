 ...
 *
 * Tests: CODEINDEX_TOOLS from tools.ts
 * - All 10 tools are defined (7 core + 3 memory)
 * - Each tool has valid name, description, inputSchema
 * - Required fields are correct
 * - Optional repo parameter is present on tools that need it
 */
import { describe, it, expect } from 'vitest';
import { CODEINDEX_TOOLS, type ToolDefinition } from '../../src/mcp/tools.js';

describe('CODEINDEX_TOOLS', () => {
  it('exports exactly 10 tools', () => {
    expect(CODEINDEX_TOOLS).toHaveLength(10);
  });

  it('contains all expected tool names', () => {
    const names = CODEINDEX_TOOLS.map(t => t.name);
  ...
  it('each tool has name, description, and inputSchema', () => {
    for (const tool of CODEINDEX_TOOLS) {
  ...
  it('query tool requires "query" parameter', () => {
    const queryTool = CODEINDEX_TOOLS.find(t => t.name === 'query')!;
  ...
  it('rename tool requires new_name', () => {
    const renameTool = CODEINDEX_TOOLS.find(t => t.name === 'rename')!;
    expect(renameTool.inputSchema.required).toContain('new_name');
  });

  it('detect_changes tool has no required parameters', () => {
    const detectTool = CODEINDEX_TOOLS.find(t => t.name === 'detect_changes')!;
    expect(detectTool.inputSchema.required).toEqual([]);
  });

  it('list_repos tool has no parameters', () => {
    const listTool = CODEINDEX_TOOLS.find(t => t.name === 'list_repos')!;
    expect(Object.keys(listTool.inputSchema.properties)).toHaveLength(0);
    expect(listTool.inputSchema.required).toEqual([]);
  });

  it('all tools except list_repos and forget have optional repo parameter', () => {
    const noRepoTools = new Set(['list_repos', 'forget']);
    for (const tool of CODEINDEX_TOOLS) {
      if (noRepoTools.has(tool.name)) continue;
