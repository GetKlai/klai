 ...
 *
 * Tests the hook scripts (codeindex-hook.cjs and codeindex-hook.js) that run
 * as PreToolUse and PostToolUse hooks in Claude Code.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Glob/Bash tool inputs
 * - findCodeIndexDir: .codeindex directory discovery
...
// ─── Test fixtures: temporary .codeindex directory ───────────────────

let tmpDir: string;
let gitNexusDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codeindex-hook-test-'));
...
describe('Windows .cmd extension handling', () => {
  // Both hooks use plain 'npx' and 'codeindex' without Windows .cmd extension
  // Windows support is handled at the OS/PATH level, not in the hook scripts
  for (const [label, hookPath] of [['CJS', CJS_HOOK]] as const) {
    it(`${label} hook invokes npx for codeindex analyze`, () => {
      const source = fs.readFileSync(hookPath, 'utf-8');
      expect(source).toContain("npx codeindex analyze");
    });
  }
});
...
describe('Git mutation regex', () => {
  ...
  // Only CJS hook has PostToolUse git mutation detection
  for (const [label, hookPath] of [['CJS', CJS_HOOK]] as const) {
...
describe('PostToolUse staleness detection (integration)', () => {
  // Plugin hook only handles PreToolUse; PostToolUse is CJS-only
  for (const [label, hookPath] of [['CJS', CJS_HOOK]] as const) {
...
describe('cwd validation (integration)', () => {
  // Plugin hook only handles PreToolUse; PostToolUse cwd validation is CJS-only
  for (const [label, hookPath] of [['CJS', CJS_HOOK]] as const) {
