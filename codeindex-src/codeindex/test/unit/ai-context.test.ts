...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.codeindex');
  ...
  it('creates or updates CLAUDE.md with CodeIndex section', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);

    const claudeMdPath = path.join(tmpDir, 'CLAUDE.md');
    const content = await fs.readFile(claudeMdPath, 'utf-8');
    expect(content).toContain('codeindex:start');
    expect(content).toContain('codeindex:end');
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    // Should only have one codeindex section
    const starts = (content.match(/codeindex:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'codeindex');
    try {
