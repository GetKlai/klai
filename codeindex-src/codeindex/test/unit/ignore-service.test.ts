...
describe('loadIgnoreRules', () => {
  ...
  it('parses .codeindexignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.codeindexignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.codeindexignore'));
  });
...
describe('loadIgnoreRules — CODEINDEX_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when CODEINDEX_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');
  ...
  it('still reads .codeindexignore when CODEINDEX_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.codeindexignore'), 'vendor/\n');
