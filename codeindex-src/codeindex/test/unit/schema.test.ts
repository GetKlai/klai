...
describe('LadybugDB Schema', () => {
  ...
  describe('relation schema', () => {
    ...
    it('has all FROM/TO pairs needed for HAS_METHOD edges', () => {
      // Class and Interface connect to both Method and Constructor
      for (const src of ['Class', 'Interface']) {
        expect(RELATION_SCHEMA).toContain(`FROM ${src} TO Method`);
        expect(RELATION_SCHEMA).toContain(`FROM ${src} TO \`Constructor\``);
      }

      // Struct connects to Method
      expect(RELATION_SCHEMA).toContain('FROM `Struct` TO Method');
    });
  });
