...
describe('processHeritageFromExtracted', () => {
  ...
  describe('extends', () => {
    it('creates EXTENDS relationship between classes', async () => {
      ...
      await processHeritageFromExtracted(graph, heritage, ctx.symbols);
  ...
  it('calls progress callback', async () => {
    ...
    await processHeritageFromExtracted(graph, heritage, ctx.symbols, onProgress);
    expect(onProgress).toHaveBeenCalledWith(1, 1);
  });

  it('handles empty heritage array', async () => {
    await processHeritageFromExtracted(graph, [], ctx.symbols);
    expect(graph.relationshipCount).toBe(0);
  });
});
