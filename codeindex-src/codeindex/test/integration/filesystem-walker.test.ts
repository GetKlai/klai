...
describe('filesystem-walker', () => {
  ...
  describe('.codeindexignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .codeindexignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.codeindexignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .codeindexignore', async () => {
  ...
  describe('combined .gitignore + .codeindexignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.codeindexignore'), 'local/\n');
    });
  ...
  describe('CODEINDEX_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when CODEINDEX_NO_GITIGNORE is set', async () => {
      const original = process.env.CODEINDEX_NO_GITIGNORE;
      process.env.CODEINDEX_NO_GITIGNORE = '1';
      try {
        const files = await walkRepositoryPaths(envDir);
        const paths = files.map(f => f.path.replace(/\\/g, '/'));
        ...
        if (original === undefined) {
          delete process.env.CODEINDEX_NO_GITIGNORE;
        } else {
          process.env.CODEINDEX_NO_GITIGNORE = original;
        }
      }
    });
  });
