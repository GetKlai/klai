 ...
 * - analyzeCommand: verify pipeline runs and creates .codeindex/ output
 *
...
afterAll(() => {
  // Clean up .git/ and .codeindex/ directories created during the test
  for (const dir of ['.git', '.codeindex']) {
    const fullPath = path.join(MINI_REPO, dir);
    if (fs.existsSync(fullPath)) {
      fs.rmSync(fullPath, { recursive: true, force: true });
    }
  }
  // Clean up global index created by the analyze test
  const globalIndex = path.join(os.homedir(), '.codeindex', 'ci-mini-repo');
  if (fs.existsSync(globalIndex)) {
    fs.rmSync(globalIndex, { recursive: true, force: true });
  }
});
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    // The tsx loader cannot resolve modules when cwd is outside the project
    // tree, so we run from the project root and pass the repo path as an arg.
    const projectName = 'ci-mini-repo';
    const result = runCliRaw(['analyze', projectName, MINI_REPO], repoRoot, 30000);
    ...
    // Successful analyze should create index in the global storage directory
    const globalDir = path.join(os.homedir(), '.codeindex', projectName);
    expect(fs.existsSync(globalDir), `expected ${globalDir} to exist`).toBe(true);
    expect(fs.statSync(globalDir).isDirectory()).toBe(true);
  });
  ...
  describe('unhappy path', () => {
    it('exits with error when no command is given', () => {
      ...
      // Commander shows help and exits with code 0 when no subcommand is given.
      // The heap re-exec wrapper propagates the child's exit code.
      expect(result.status).toBe(0);
      const combined = result.stdout + result.stderr;
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/codeindex/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // finds the parent project's .codeindex. Use an isolated temp git
      // repo to guarantee no .codeindex exists anywhere in the path.
  ...
  // Requires analyze to have run first (the analyze test above populates .codeindex/).
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('CODEINDEX_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('CODEINDEX_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
