 ...
 *
 * Tests the hook scripts with real git repos and .codeindex directories.
...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'codeindex-hook.cjs');
const PLUGIN_HOOK = path.resolve(__dirname, '..', '..', '..', 'codeindex-claude-plugin', 'hooks', 'codeindex-hook.js');
...
beforeAll(() => {
  ...
  gitNexusDir = path.join(tmpDir, '.codeindex');
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  // Plugin hook only handles PreToolUse; PostToolUse is only in the CJS hook
  describe.skipIf(name === 'Plugin')('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      expect(output!.additionalContext).toContain('npx codeindex analyze');
    });
  ...
  describe('PreToolUse — silent without codeindex CLI', () => {
    // PreToolUse tries to spawn `codeindex augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `codeindex augment` hangs and the 10s timeout kills the process.
  ...
  describe('directory without .codeindex', () => {
    // The hook walks up 5 parent directories looking for .codeindex.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .codeindex could exist in any ancestor.
    let noCodeIndexDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .codeindex
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-codeindex-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noCodeIndexDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noCodeIndexDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noCodeIndexDir, stdio: 'pipe' });
    });

    afterAll(() => {
      // Clean up from the base directory
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, path.basename(path.resolve(noCodeIndexDir, '..', '..', '..', '..', '..', '..')));
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .codeindex directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .codeindex directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
