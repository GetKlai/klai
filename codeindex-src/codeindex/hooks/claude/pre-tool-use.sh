#!/bin/bash
# CodeIndex PreToolUse hook for Claude Code
...
# Check if we're in a CodeIndex-indexed repo
dir="${CWD:-$PWD}"
found=false
for i in 1 2 3 4 5; do
  if [ -d "$dir/.codeindex" ]; then
    found=true
    break
  fi
  parent="$(dirname "$dir")"
...
# Run codeindex augment — must be fast (<500ms target)
RESULT=$(cd "$CWD" && npx -y codeindex augment "$PATTERN" 2>/dev/null)
