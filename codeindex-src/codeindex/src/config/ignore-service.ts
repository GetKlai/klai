...
// CJS interop: `ignore` uses module.exports = fn which doesn't resolve
// cleanly under NodeNext ESM resolution. Use createRequire as workaround.
const _require = createRequire(import.meta.url);
const ignore = _require('ignore') as (options?: { ignorecase?: boolean }) => Ignore;
import fs from 'fs/promises';
import nodePath from 'path';
import type { Path } from 'path-scurry';

/** Directory names that should be skipped entirely during traversal */
export const DEFAULT_IGNORE_DIRS = [
    ...
    'site-packages', '.tox', 'eggs', '.eggs', 'lib64', 'parts', 'sdist', 'wheels',
    // Build Outputs
...
// NOTE: Negation patterns in .codeindexignore (e.g. `!vendor/`) cannot override
...
/**
 * Load .gitignore and .codeindexignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .codeindexignore. Defaults to CODEINDEX_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.CODEINDEX_NO_GITIGNORE;
  const filenames = skipGitignore
    ? ['.codeindexignore']
    : ['.gitignore', '.codeindexignore'];
 ...
 * - .gitignore / .codeindexignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .codeindexignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // Check against .gitignore / .codeindexignore patterns.
