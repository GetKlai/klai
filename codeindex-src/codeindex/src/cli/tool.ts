 ...
 * 
 * Exposes CodeIndex tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 * 
 * Usage:
 *   codeindex query "authentication flow"
 *   codeindex context --name "validateUser"
 *   codeindex impact --target "AuthService" --direction upstream
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('CodeIndex: No indexed repositories found. Run: codeindex analyze');
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `codeindex cypher ... | head -1`)
...
export async function queryCommand(queryText: string, options?: {
  ...
  if (!queryText?.trim()) {
    console.error('Usage: codeindex query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(name: string, options?: {
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: codeindex context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(target: string, options?: {
  ...
  if (!target?.trim()) {
    console.error('Usage: codeindex impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: 'Try reducing --depth or using codeindex context <symbol> as a fallback',
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(query: string, options?: {
  ...
  if (!query?.trim()) {
    console.error('Usage: codeindex cypher <cypher_query>');
    process.exit(1);
  }
