 ...
 * 
 * Provides tool implementations using local .codeindex/ indexes.
...
import { initKuzu, executeQuery, closeKuzu, isKuzuReady } from '../core/lbug-adapter.js';
...
import {
  ...
  ensureProjectMemory,
  ensureMemory,
} from '../../core/memory/global-store.js';
import {
  createObservation,
  searchObservations,
...
import type { ObservationType, ObservationScope, RecallScope } from '../../core/memory/types.js';
// AI context generation is CLI-only (codeindex analyze)
// import { generateAIContextFiles } from '../../cli/ai-context.js';

/** Regex matching Cypher write operations — used to block mutations in read-only mode. */
export const CYPHER_WRITE_RE = /\b(CREATE|DELETE|SET|MERGE|REMOVE|DROP|ALTER|COPY|DETACH)\b/i;

/** Returns true when a Cypher query contains a write operation. */
export function isWriteQuery(query: string): boolean {
  return CYPHER_WRITE_RE.test(query);
}

/** Valid relationship types for safe Cypher query construction */
export const VALID_RELATION_TYPES = new Set([
  'CALLS', 'IMPORTS', 'EXTENDS', 'IMPLEMENTS',
  'HAS_METHOD', 'HAS_PROPERTY', 'OVERRIDES', 'ACCESSES',
]);
...
/**
 * Trim content to a short signature (first 4 lines + ellipsis).
 * Keeps MCP responses compact — Claude can read full source via file locations.
 */
function truncateContent(raw: string, maxLines = 4): string {
...
export class LocalBackend {
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    // Still no match — try CWD-based auto-detection for multi-repo setups
    if (!repoParam && this.repos.size > 1) {
      try {
        const cwd = process.cwd();
        const { getMainRepoRoot, getGitRoot } = await import('../../storage/git.js');
        const mainRoot = getMainRepoRoot(cwd);
        const gitRoot = getGitRoot(cwd);

        for (const handle of this.repos.values()) {
          const handlePath = path.resolve(handle.repoPath);
          if ((mainRoot && handlePath === mainRoot) || (gitRoot && handlePath === gitRoot)) {
            return handle;
          }
        }
      } catch {
        // CWD detection failed — fall through to error
      }
    }

    // No match — throw with helpful message
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: codeindex analyze');
    }
    if (repoParam) {
  ...
  private async ensureInitialized(repoId: string): Promise<void> {
    ...
    try {
      await initKuzu(repoId, handle.lbugPath);
  ...
  async callTool(method: string, params: any): Promise<any> {
    ...
    // Memory tools work without a repo (global scope)
    if (method === 'remember' || method === 'recall' || method === 'forget') {
      let repo: RepoHandle | null = null;
      try {
        repo = await this.resolveRepo(params?.repo);
      } catch {
        // No repo available — memory tools still work (global scope)
      }
      switch (method) {
        case 'remember': return this.remember(repo, params);
        case 'recall': return this.recall(repo, params);
        case 'forget': return this.forget(params);
      }
    }
  ...
  /**
   * Remember — save an observation to persistent memory.
   */
  private async remember(repo: RepoHandle | null, params: any): Promise<any> {
    const title = params.title as string;
    const content = params.content as string;
    const type = params.type as ObservationType;
    const scope = (params.scope || 'repo') as ObservationScope;
    const tags = (params.tags || []) as string[];
    const refNames = (params.refs || []) as string[];

    if (!title || !content || !type) {
      return { error: 'Missing required params: title, content, type' };
    }

    // Determine storage target
    const isGlobal = scope === 'global';
    const projectName = isGlobal ? GLOBAL_PROJECT_NAME : (repo?.name || GLOBAL_PROJECT_NAME);
    const dbKey = projectName;

    // Ensure memory DB is ready
    await ensureMemory(isGlobal ? undefined : projectName);

    // Resolve refs to symbol info (best-effort)
    const resolvedRefs: Array<{ refType: 'symbol' | 'file'; refId: string; refName: string }> = [];
    if (!isGlobal && repo && refNames.length > 0) {
      for (const refName of refNames) {
        if (refName.includes('/') || refName.includes('.')) {
          // Looks like a file path
          resolvedRefs.push({ refType: 'file', refId: refName, refName: refName.split('/').pop() || refName });
        } else {
          // Try to resolve as a symbol in the code graph
          try {
            await this.ensureInitialized(repo.id);
            const rows = await executeQuery(repo.id, `
              MATCH (n) WHERE n.name = '${refName.replace(/'/g, "\\'")}' RETURN n.id as id, n.name as name, labels(n) as labels LIMIT 1
            `);
            if (rows.length > 0) {
              resolvedRefs.push({ refType: 'symbol', refId: rows[0].id, refName: rows[0].name });
            } else {
              // Symbol not found in graph — store as-is
              resolvedRefs.push({ refType: 'symbol', refId: refName, refName });
            }
          } catch {
            // Graph not available — store ref name as-is
            resolvedRefs.push({ refType: 'symbol', refId: refName, refName });
          }
        }
      }
    ...
    const observation = await createObservation(dbKey, {
      name: title,
      type,
      content,
      tags,
      project: projectName,
      refs: resolvedRefs.length > 0 ? resolvedRefs : undefined,
    });

    // Embed for semantic search (best-effort, don't fail the save)
    try {
      const { embedObservation } = await import('../../core/memory/observation-embedder.js');
      await embedObservation(dbKey, observation.uid, title, content);
    } catch {
      // Embedding may fail (model not downloaded, etc.) — observation is still saved
    }

    const refsText = resolvedRefs.length > 0
      ? `\nLinked to: ${resolvedRefs.map(r => `${r.refName} (${r.refType})`).join(', ')}`
      : '';
    ...
    return {
      ...
      scope: isGlobal ? 'global' : projectName,
      message: `[${type}] "${title}" saved to ${isGlobal ? 'global' : projectName} memory.${refsText}`,
    };
  }

  /**
   * Recall — search observations in persistent memory.
   */
  private async recall(repo: RepoHandle | null, params: any): Promise<any> {
    const query = params.query as string | undefined;
    const type = params.type as ObservationType | undefined;
    const scope = (params.scope || 'all') as RecallScope;
    ...
    // Use hybrid search (text + semantic) when query is provided
    const searchFn = query ? hybridSearchObservations : searchObservations;

    // Search global memory
    if (scope === 'all' || scope === 'global') {
      try {
        await ensureGlobalMemory();
        const globalResults = await searchFn(GLOBAL_PROJECT_NAME, {
          query,
          type,
          days,
          limit,
        ...
        for (const r of globalResults) {
          results.push({
            ...
            title: r.observation.name,
            type: r.observation.type,
            content: r.observation.content,
            tags: r.observation.tags,
            scope: 'global',
            created: r.observation.createdAt,
            refs: r.refs.map(ref => `${ref.refName} (${ref.refType})`),
          });
        }
      } catch {
        // Global memory not available — continue
      }
    }

    // Search project memory
    if ((scope === 'all' || scope === 'repo') && projectName) {
      try {
        await ensureProjectMemory(projectName);
        const projectResults = await searchFn(projectName, {
          query,
          type,
          project: projectName,
          days,
        ...
        // Project memory not available — continue
      }
    }

    // Sort by creation date (newest first) and cap at limit
  ...
  /**
   * Forget — archive or permanently delete an observation.
   */
  private async forget(params: any): Promise<any> {
    ...
    // Try to find the observation in global memory first, then all projects
    const dbKeys = [GLOBAL_PROJECT_NAME, ...Array.from(this.repos.keys())];

    for (const dbKey of dbKeys) {
      try {
        await ensureMemory(dbKey === GLOBAL_PROJECT_NAME ? undefined : dbKey);

        if (permanent) {
          const deleted = await deleteObservation(dbKey, uid);
          if (deleted) {
            return { status: 'deleted', uid, message: `Observation ${uid} permanently deleted.` };
          }
        } else {
          const archived = await archiveObservation(dbKey, uid);
          if (archived) {
            return { status: 'archived', uid, message: `Observation ${uid} archived.` };
          }
        }
      } catch {
        continue;
    ...
    return { error: `Observation ${uid} not found.` };
  }
  ...
  private async query(repo: RepoHandle, params: {
    ...
    const maxSymbolsPerProcess = params.max_symbols || 8;
    ...
    for (const [_, item] of merged) {
      ...
      // Optionally fetch content (signature only — first 4 lines to keep responses compact)
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('CodeIndex: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
  ...
  private async context(repo: RepoHandle, params: {
    ...
    // Memory observations linked to this symbol
    let observations: any[] = [];
    try {
      const symbolName = sym.name || sym[1];
      const symbolId = sym.id || sym[0];
      await ensureProjectMemory(repo.name);
      const { searchObservations: searchObs } = await import('../../core/memory/observation-store.js');
      // Search for observations that reference this symbol by name
      const obsResults = await searchObs(repo.name, { query: symbolName, limit: 5 });
      observations = obsResults.map(r => ({
        uid: r.observation.uid,
        title: r.observation.name,
        type: r.observation.type,
        age: formatAge(r.observation.createdAt),
      }));
    } catch {
      // Memory not available — that's fine
    }
    ...
    return {
      ...
      symbol: {
        ...
        startLine: sym.startLine || sym[4],
        endLine: sym.endLine || sym[5],
        ...(include_content && (sym.content || sym[6]) ? { content: truncateContent(sym.content || sym[6]) } : {}),
      },
  ...
  private async detectChanges(repo: RepoHandle, params: {
    ...
    // Auto-save change observation to memory
    if (params.save_observation && changedSymbols.length > 0) {
      try {
        const symbolNames = changedSymbols.slice(0, 5).map((s: any) => s.name).join(', ');
        const processNames = Array.from(affectedProcesses.values()).slice(0, 3).map((p: any) => p.name).join(', ');
        const title = `Changes: ${changedSymbols.length} symbols, ${risk} risk`;
        const content = `${changedFiles.length} files changed. Symbols: ${symbolNames}${changedSymbols.length > 5 ? '...' : ''}.${processNames ? ` Affected flows: ${processNames}.` : ''}`;
        await this.remember(repo, {
          title,
          content,
          type: 'note',
          scope: 'repo',
          tags: ['change-detection', risk],
          refs: changedSymbols.slice(0, 3).map((s: any) => s.name),
        });
        (result as any).observation_saved = true;
      } catch {
        // Don't fail the detect_changes if memory save fails
      }
    }
  ...
  private async impact(repo: RepoHandle, params: {
    ...
    try {
      ...
      return {
        ...
        impactedCount: 0,
        risk: 'UNKNOWN',
        suggestion: 'The graph query failed — try codeindex context <symbol> as a fallback',
      };
    }
  }
  ...
  private async _impactImpl(repo: RepoHandle, params: {
    ...
    const relationTypes = params.relationTypes && params.relationTypes.length > 0
      ...
      : ['CALLS', 'IMPORTS', 'EXTENDS', 'IMPLEMENTS', 'HAS_METHOD', 'OVERRIDES'];
