 ...
 * then looks up the global registry.
 */
async function findRepoForCwd(cwd: string): Promise<{
  ...
  try {
    const entry = await findRegistryEntry(cwd);
...
export async function augment(pattern: string, cwd?: string): Promise<string> {
  ...
  try {
    ...
    const { initKuzu, executeQuery, isKuzuReady } = await import('../../mcp/core/lbug-adapter.js');
    ...
    if (!isKuzuReady(repoId)) {
      await initKuzu(repoId, repo.lbugPath);
    }
    ...
    const lines: string[] = [`[CodeIndex] ${enriched.length} related symbols found:`, ''];
