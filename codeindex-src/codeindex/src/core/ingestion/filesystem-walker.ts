...
import { shouldIgnorePath, DEFAULT_IGNORE_DIRS, createIgnoreFilter } from '../../config/ignore-service.js';
 ...
 *
 * Uses streaming glob so the caller sees progress during file discovery
 * instead of a long blank wait.
 */
export const walkRepositoryPaths = async (
  repoPath: string,
  onProgress?: (current: number, total: number, filePath: string) => void,
  onDiscovery?: (found: number) => void
): Promise<ScannedFile[]> => {
  // Stream files so we can report discovery progress
  // Use createIgnoreFilter to combine .gitignore/.codeindexignore with hardcoded rules.
  // This enables directory-level pruning during traversal, avoiding descent into
  // heavy dirs like node_modules (500K+ files) or git-ignored directories.
