...
interface SwiftPackageConfig {
  /** Map of target name -> source directory path (e.g., "MyModule" -> "Sources/MyModule") */
  targets: Map<string, string>;
}

async function loadSwiftPackageConfig(repoRoot: string): Promise<SwiftPackageConfig | null> {
  // Swift imports are module-name based (e.g., `import MyModule`)
...
const resolveImportPath = (
  ...
  if (language === SupportedLanguages.Python) {
    const pyResolved = resolvePythonImport(currentFile, importPath, allFiles);
    if (pyResolved) return cache(pyResolved);
    if (importPath.startsWith('.')) return cache(null); // relative but unresolved — don't suffix-match
  }
 ...
 *
 * 1. Relative (PEP 328): `.module`, `..module` — 1 dot = current package,
 *    each extra dot goes up one level.
 * 2. Proximity bare import: static heuristic — checks the importer's own
 *    directory first. Single-segment only — multi-segment (e.g. `os.path`)
 *    falls through to suffixResolve.
 *    Checks package (__init__.py) before module (.py), matching CPython's
 *    finder order (PEP 451).
 *
...
function resolvePythonImport(
  ...
  // Proximity bare import — single-segment only; package before module (PEP 451)
...
/**
 * Resolve a Ruby require/require_relative import to a file.
 * require 'foo/bar' → tries suffix match for foo/bar.rb
 * require_relative './foo' → resolved by caller before arriving here
 */
function resolveRubyImport(
  ...
  // Ruby require paths are typically suffix-matchable: require 'models/user' → models/user.rb
  const pathParts = importPath.replace(/\\/g, '/').split('/').filter(Boolean);
...
const KOTLIN_EXTENSIONS: string[] = ['.kt', '.kts'];

/**
 * Resolve a JVM wildcard import (com.example.*) to all matching source files.
 * Used for both Java (.java) and Kotlin (.kt, .kts).
...
function resolveJvmWildcard(
  ...
  if (index) {
    // Use directory index: get all matching files in this package directory
    const candidates: string[] = [];
    for (const ext of exts) {
      candidates.push(...index.getFilesInDir(packagePath, ext));
    }
  ...
  for (let i = 0; i < normalizedFileList.length; i++) {
    const normalized = normalizedFileList[i];
    if (normalized.includes(packageSuffix) && exts.some(ext => normalized.endsWith(ext))) {
...
/**
 * Try to resolve a JVM member import by stripping the member name.
 * "com.example.Constants.VALUE" -> resolve "com.example.Constants"
 * Used for both Java static imports and Kotlin top-level member imports.
 */
function resolveJvmMemberImport(
  ...
  // Static/member imports look like: com.example.Constants.VALUE or com.example.Constants.*
...
export const processImports = async (
  ...
  // Track skipped languages for verbose logging
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    matches.forEach(match => {
      ...
      if (captureMap['import']) {
        ...
        // Kotlin: detect wildcard imports (import com.example.*)
        // tree-sitter-kotlin captures the identifier without the .* suffix,
        // so check for a ".*" child node in the import node.
        if (language === SupportedLanguages.Kotlin && !rawImportPath.endsWith('.*')) {
          const importNode = captureMap['import'];
          for (let ci = 0; ci < importNode.childCount; ci++) {
            const child = importNode.child(ci);
            if (child && child.type === '.*') {
              rawImportPath += '.*';
              break;
            }
        ...
        if (language === SupportedLanguages.Java || language === SupportedLanguages.Kotlin) {
          ...
          if (rawImportPath.endsWith('.*')) {
            let matchedFiles = resolveJvmWildcard(rawImportPath, normalizedFileList, allFileList, exts, index);
            // Kotlin: also try .java files if no .kt matches found (cross-language JVM imports)
            if (matchedFiles.length === 0 && language === SupportedLanguages.Kotlin) {
              matchedFiles = resolveJvmWildcard(rawImportPath, normalizedFileList, allFileList, ['.java'], index);
            }
            for (const matchedFile of matchedFiles) {
              addImportEdge(file.path, matchedFile);
            }
            if (matchedFiles.length > 0) return; // skip single-file resolution
          }

          // Try member import resolution (strip member name)
          ...
          // Fall through to normal resolution for regular JVM imports
        }
        ...
        if (language === SupportedLanguages.Swift && swiftPackageConfig) {
          // Swift imports are module names: `import MyModule`
      ...
      // ---- Ruby: require / require_relative are call nodes, not import nodes ----
          ...
          if (calledName === 'require' || calledName === 'require_relative') {
            ...
            if (contentNode) {
              ...
              if (importPath && importPath.length <= 1024 && !/[\x00-\x1f]/.test(importPath)) {
                ...
                if (isRelative) {
                  // require_relative uses standard relative resolution
                  const resolvedPath = resolveImportPath(
                    file.path, importPath, allFilePaths, allFileList, normalizedFileList,
                    resolveCache, language, tsconfigPaths, index,
                  );
                  if (resolvedPath) addImportEdge(file.path, resolvedPath);
                } else {
                  // require uses suffix-based resolution
                  const resolved = resolveRubyImport(importPath, normalizedFileList, allFileList, index);
...
export const processImportsFromExtracted = async (
  ...
  for (const [filePath, fileImports] of importsByFile) {
    ...
    for (const { rawImportPath, language } of fileImports) {
      ...
      // Ruby: require / require_relative (pre-extracted from call nodes by worker)
      if (language === SupportedLanguages.Ruby) {
        if (rawImportPath.startsWith('.')) {
          // require_relative — use standard relative resolution
          const resolved = resolveImportPath(
            filePath, rawImportPath, allFilePaths, allFileList, normalizedFileList,
            resolveCache, language as SupportedLanguages, tsconfigPaths, index,
          );
          if (resolved) {
            resolveCache.set(cacheKey, resolved);
            addImportEdge(filePath, resolved);
          ...
          // require — suffix-based resolution
