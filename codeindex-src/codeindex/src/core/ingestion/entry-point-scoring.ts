...
const ENTRY_POINT_PATTERNS: Record<string, RegExp[]> = {
  ...
  'c': [
    ...
    /^init_/,                 // Initialization functions (init_server)
    /_init$/,                 // Reversed init patterns (server_init)
    /^start_/,                // Start functions (start_server)
    /_start$/,                // Reversed start patterns (server_start)
    /^run_/,                  // Run functions
    /^handle_/,               // Handler functions (handle_request)
    /_handler$/,              // Handler suffix (signal_handler)
    /_callback$/,             // Callback functions (event_callback)
    /^cmd_/,                  // Command functions (cmd_new_window)
    /_connect$/,              // Connection functions (client_connect)
    /_create$/,               // Creation functions (session_create)
    /_resize$/,               // Resize handlers (window_resize)
    /^signal_/,               // Signal handling
    /^event_/,                // Event handling
  ],
  ...
  'cpp': [
    /^main$/,                 // THE entry point
    /^init_/,
    /^Create[A-Z]/,           // Factory patterns (CreateInstance)
    /^create_/,               // Snake_case factory (create_session)
    /^[Rr]un$/,               // Run/run methods
    /^[Ss]tart$/,             // Start/start methods
    /^On[A-Z]/,               // Event handlers (OnEventReceived)
    /^on_/,                   // Snake_case event handlers (on_click)
  ],
