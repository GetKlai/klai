...
const resolveExtendsType = (
  ...
  // Check symbol table for authoritative type info
  const exactFull = symbolTable.lookupExactFull(currentFilePath, parentName);
  if (exactFull) {
    const isInterface = exactFull.type === 'Interface';
    return isInterface
      ? { type: 'IMPLEMENTS', idPrefix: 'Interface' }
      : { type: 'EXTENDS', idPrefix: 'Class' };
  }
  const fuzzyResults = symbolTable.lookupFuzzy(parentName);
  if (fuzzyResults.length > 0) {
    const isInterface = fuzzyResults[0].type === 'Interface';
    return isInterface
      ? { type: 'IMPLEMENTS', idPrefix: 'Interface' }
      : { type: 'EXTENDS', idPrefix: 'Class' };
  }
 ...
 * Uses symbolTable lookupExact -> lookupFuzzy -> generate synthetic ID.
 */
const resolveHeritageId = (
  ...
  const exact = symbolTable.lookupExact(filePath, name);
  if (exact) return exact;
  const fuzzy = symbolTable.lookupFuzzy(name);
  if (fuzzy.length === 1) return fuzzy[0].nodeId;
  // For global with multiple candidates, refuse (a wrong edge is worse than no edge)
  if (fuzzy.length > 1) return generateId(fallbackLabel, fallbackKey ?? name);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    matches.forEach(match => {
      ...
      if (captureMap['heritage.class'] && captureMap['heritage.extends']) {
        ...
        const { type: relType, idPrefix } = resolveExtendsType(parentClassName, file.path, symbolTable, language);

        const childId = resolveHeritageId(className, file.path, symbolTable, 'Class', `${file.path}:${className}`);
        const parentId = resolveHeritageId(parentClassName, file.path, symbolTable, idPrefix);
      ...
      if (captureMap['heritage.class'] && captureMap['heritage.implements']) {
        ...
        const classId = resolveHeritageId(className, file.path, symbolTable, 'Class', `${file.path}:${className}`);
        const interfaceId = resolveHeritageId(interfaceName, file.path, symbolTable, 'Interface');
      ...
      if (captureMap['heritage.trait'] && captureMap['heritage.class']) {
        ...
        const structId = resolveHeritageId(structName, file.path, symbolTable, 'Struct', `${file.path}:${structName}`);
        const traitId = resolveHeritageId(traitName, file.path, symbolTable, 'Trait');
...
export const processHeritageFromExtracted = async (
  ...
  for (let i = 0; i < extractedHeritage.length; i++) {
    ...
    if (h.kind === 'extends') {
      ...
      const { type: relType, idPrefix } = resolveExtendsType(h.parentName, h.filePath, symbolTable, fileLanguage);

      const childId = resolveHeritageId(h.className, h.filePath, symbolTable, 'Class', `${h.filePath}:${h.className}`);
      const parentId = resolveHeritageId(h.parentName, h.filePath, symbolTable, idPrefix);
      ...
      const classId = resolveHeritageId(h.className, h.filePath, symbolTable, 'Class', `${h.filePath}:${h.className}`);
      const interfaceId = resolveHeritageId(h.parentName, h.filePath, symbolTable, 'Interface');
      ...
      const structId = resolveHeritageId(h.className, h.filePath, symbolTable, 'Struct', `${h.filePath}:${h.className}`);
      const traitId = resolveHeritageId(h.parentName, h.filePath, symbolTable, 'Trait');
