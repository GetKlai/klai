 ...
 * 
 * Encapsulated Cypher queries against the CodeIndex knowledge graph.
 * Uses the MCP-style pooled lbug-adapter for connection management.
 */

import { initKuzu, executeQuery, closeKuzu } from '../../mcp/core/lbug-adapter.js';
...
export async function initWikiDb(lbugPath: string): Promise<void> {
  await initKuzu(REPO_ID, lbugPath);
}
