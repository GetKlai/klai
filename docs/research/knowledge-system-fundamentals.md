...
| 12 | [Communicatie als kennisbron](#bevinding-12-communicatie-als-kennisbron) | Verplaatst naar `communicatie-als-kennisbron.md` (basisdocument); hier alleen nog de kern-samenvatting |
...
Deze bevinding is uitgegroeid tot een eigen basisdocument en daarheen verplaatst: **`communicatie-als-kennisbron.md`**. Daar staan de vijf communicatietypen met hun extractie-aanpak, de pipeline-tabel, de onafhankelijk getoetste Cerebras-casestudy, de datamodel-basis (conversation/message/participant + canonical entities) en de verwijzingen naar de verdieping (telefonie-keten in `knowledge-pipeline-architecture.md` §5.5; vergelijkingsrapport `rag-vergelijking-superdock-engram-klai.md`).

De kern in één alinea: communicatie is naast documenten en websites de tweede grote ingest-bron. Elk type — e-mail, chat, calls, vergaderingen, 1-op-1 — heeft een eigen signaal/ruis-verhouding en vraagt een type-specifieke extractiepipeline; de extractie-eenheid is de thread, de call of het transcript, nooit het losse bericht. Structuur bewaren gaat vóór extractie (weggegooide threadstructuur is onomkeerbaar; extractie kun je altijd later toevoegen), en de kwaliteit wordt — zoals overal in dit document — bepaald bij opslag, niet bij zoeken.
...
Korte berichten (chat, e-mail) mogen nooit individueel geïndexeerd worden — een bericht van 6 woorden produceert een onbruikbare "noisy averaged" embedding. Thread-aggregatie is verplicht (zie `communicatie-als-kennisbron.md`).
