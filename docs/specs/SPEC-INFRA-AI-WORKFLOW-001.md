...
**Restored:** 2026-09-15 — this file was deleted as collateral by `8f29b6fc9`
("Remove remaining MoAI/agency framework dirs") while the four hooks that cite
it stayed wired and running. It is restored verbatim from `57937ea35` at the
repo's current spec path. The hooks now name this path so the next cleanup can
see what it would be orphaning; `rules/tests/test_code_intelligence_wiring.py`
fails if it disappears again.
