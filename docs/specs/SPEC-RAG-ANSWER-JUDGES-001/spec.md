...
3. De vraag-judge stuurt de generatie niet: er gaat geen algemene opdracht mee om door te vragen. Zo'n opdracht maakte in v0.5.0 het eerste antwoord slechter en leverde bijna geen vragen op (2 op 150), en ook gemeten op de beurt erna hielp hij niet (2.46). Dat zegt iets over een opdracht aan het antwoordmodel, niet over doorvragen zelf: een aparte stap die de vraag uit de gevonden artikelen kiest, wint wel (regel 10). Onduidelijkheid volgens de vraag-judge labelt alleen een concept dat op een vraagteken eindigt.
...
8. De interne chat draait dezelfde controle én dezelfde reparatie, met dezelfde tekst, hetzelfde schema en dezelfde drempel uit `klai-libs/chat-prompts`, voor elke tenant en zonder poort per organisatie (#1526, #1530). De reparatie draait overal waar het hele antwoord nog in handen is: het niet-streamende pad en de Strict-stroom, die de renderer volledig vasthoudt tot het slotframe. Een Open-stroom stuurt de woorden zoals ze komen en wordt niet gecontroleerd en ook niet gemeten: `_measure_answer_grounding` stopt bij niet-Strict (correctie 2026-09-24). Budget intern 12 s controle en 8 s reparatie (tegen 4 en 3 op de widget), omdat de antwoorden drie keer zo lang zijn; loopt een limiet af, dan blijft het antwoord zoals het was. Anders dan op de widget wordt intern nooit geweigerd op dit signaal: blijft er niets over, dan blijft het antwoord staan en logt de stap dat.
...
10. Vóór het schrijven kiest een aparte stap (`answer_plan`, `klai-fast`, 2 s) op basis van het gesprek en de gevonden artikelen tussen direct antwoorden, een storing diagnosticeren en procedures onderscheiden. Bij de laatste twee krijgt het antwoordmodel één vraag met twee tot vier opties die letterlijk uit de artikelen moeten komen (2.47, #1620).
11. Zijn alle bronnen zwak (reranker onder 0,4) en plande regel 10 geen vraag, dan gebruikt het antwoordmodel de artikelen alleen als er één de vraag letterlijk beantwoordt, en zegt het anders eerlijk dat het er niet staat, met de afspraakknop (2.48, #1635).
