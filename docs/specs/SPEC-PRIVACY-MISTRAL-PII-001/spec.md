...
| `PHONE_NUMBER` | libphonenumber, EU/EEA regions prepended to the stock list | `NLPhoneRecognizer` — a subclass, **not** a YAML `supported_regions:` key, which the registry loader silently drops (see ~~A6~~). NL+BE only until 2026-08-25 |

**Jurisdiction coverage is not uniform, and the gaps are the point.** As of
2026-08-25 the pack detects EU-wide for VAT, phone, IBAN, credit card, email and
credentials; NL+BE for company registration numbers; and NL only for postcode and
BSN. A tenant outside the Netherlands gets most of the return set and no address
