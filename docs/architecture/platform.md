...
`api.getklai.com/partner/v1/*` exposes the platform to external developers (SPEC-API-001). Endpoints: `/chat/completions`, `/models`, `/feedback`, `/knowledge/append`, `/widget-config`. Bearer auth with `pk_live_<40-hex>` tokens, SHA-256 hashed in `partner_api_keys`. Every key carries a KB whitelist, per-key rate limit, and explicit permissions. `/chat/completions` is the canonical OpenAI-compatible chat path: requests with Klai knowledge extensions (`knowledge`, `knowledge_base_ids`, `page_context`, or web-search fields) use the knowledge-grounded RAG flow and require `chat`; plain OpenAI chat requests use the general passthrough and require `general_chat`. The general passthrough forwards only OpenAI chat fields and enforces a 128 KiB request body, `n=1`, bounded output tokens, estimated input tokens <= 16k, route RPM 10, and estimated TPM 60k. Portal forwards these general calls with `LITELLM_GENERAL_CHAT_KEY`, a dedicated LiteLLM virtual key that must be distinct from `LITELLM_MASTER_KEY` and must also carry budget/RPM/TPM limits.
