...
| [chat-system.md](chat-system.md) | **Engineering reference (NL), verified 2026-09-24** — how both chat paths work today (internal chat via LiteLLM, widget via portal-api): every step, every LLM call, where answer/clarify/refuse is decided, what is measured, and what was tried and rejected. Read before changing the chat path |
| [chat-quality-history-and-plan.md](chat-quality-history-and-plan.md) | Chat quality (NL): what was researched and changed since March with how each result was measured, what external research says, and the improvement plan |
| [knowledge-retrieval-flow.md](knowledge-retrieval-flow.md) | **Engineering reference** (Part 1 on the chat request path is out of date, see chat-system.md) — the running retrieval system: LiteLLM hook (`klai_kb_*` modules), retrieval pipeline, strict/open answer policy, context injection, UI toggles, config values |
