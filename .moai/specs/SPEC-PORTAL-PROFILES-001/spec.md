...
| 2026-05-03 | 0.6.0 | Phase 3 merged via PR #283 (squash commit `72521447`). Productiebug ontdekt: na de Phase-3 verwijdering van de admin-bypass uit `routes/app/route.tsx` was de sidebar leeg voor admins met enkel plan-included producten. Oorzaak: `get_effective_products` (in `app/services/entitlements.py`) unioneerde alleen direct + group toekenningen, geen plan-included producten uit `PLAN_PRODUCTS`. Fix: `get_effective_products` resolveert nu `(org_id, plan)` in één lookup en unioneert `PLAN_PRODUCTS[plan]` ∪ direct ∪ group. Add-on producten (scribe, docs) zitten NIET in `PLAN_PRODUCTS` en blijven dus per-tenant `enabled_addons` + per-user/group toekenning vereisen — geen impact op de twee-laags `require_product`. Bestaande tests in `test_entitlements_self_heal.py` en `test_entitlements_rls.py` aangepast voor de nieuwe `(org_id, plan)` row-shape. Vier nieuwe tests dekken plan-included only, plan + user + group union, en dedup-bij-overlap. |
| 2026-05-04 | 0.7.0 | **SHIPPED.** Final post-merge polish ronde. PRs gemerged sinds v0.6.0: #285 (plan-products fix, squash `2d09c013`), #286 (KlaiTenantHostMiddleware cherry-pick uit verlaten branch, squash `a65f9e43`), #288 (em-dashes weg + add-ons Save-button pattern + tools-grid hide-not-grey, squash deel van Phase-3 polish), #289 (alembic-DROP-TABLE fix → post_deploy SQL pattern, squash `d501e2a6`), #290 (R2 — dead `maybe_generate_proposal` callsite weg, squash `faf9023d`), #291 (add-on assignment: `list_available_products` + `assign_product` accepteren `enabled_addons`, plus `get_effective_products` filtert dormant add-ons, squash `db9c2e90`), #292 (R3 — `portal_taxonomy_nodes.doc_count` kolom gedropt, squash `5d8465f9`). Productie incident tijdens AUTH-009 deploy: `op.drop_table` van klai-owned `portal_org_allowed_domains` faalde met InsufficientPrivilegeError → portal-api crash-loop → 502. Recovery handmatig (DROP als klai, ADD COLUMN, stamp alembic). Pitfall gedocumenteerd als `alembic-cannot-drop-non-portal_api-tables (HIGH)`. Drie post_deploy SQL files (SPEC-WIDGET-002, SPEC-INFRA-TENANT-DELETE-001, RLS-raise-on-missing-context) toegepast op prod als opruim. PR #259 pytest warnings (AsyncMock add coroutine never awaited) opgelost in 3 tests. Alle openstaande PRs verwerkt: 17 gemerged (Phase 1/1.6/2/3 + 8× SPEC-SEC-AUDIT-2026-04 + tenant-host-guard + AUTH-009 + add-on assignment + R2/R3 cleanup + post-deploy alembic fix), 4 gesloten als obsolete (#90 SPEC-KB-027, #103 chat-first IA, #118 connector cleanup, #151 ruff E402, #261 tenant-host-guard duplicate). |
