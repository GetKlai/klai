...
| 0.5.0 | 2026-05-25 | orchestrator | **Window 3 (REQ-10, REQ-12, REQ-13, REQ-14, REQ-15, REQ-16, REQ-18) IMPLEMENTED on feature branch `feature/SPEC-SEC-CROSS-TENANT-FOLLOWUP-001-window-3`.** REQ-11 already closed by Window 2 REQ-4. REQ-17 + REQ-19 remain as cross-repo klai-infra deliverables. Backend pytest 2856 passing (Window 2 ended at 2843 → +13 net new). Alembic single head `874c2c370830`. Per-REQ changes: REQ-10 `tenant_scoped_session(org_row.id)` for create-tenant owner-INSERT + `cross_org_session` cleanup on owner-INSERT failure (request session no longer carries `set_tenant` GUC); REQ-12 `UserPermissions.status` field plus 403 `user_suspended` in `_resolve_caller_with_options`, `zitadel.lock_user/unlock_user` after suspend/reactivate commit with desync audit + `SuspendResponse.zitadel_sync_failed`; REQ-13 `Depends(require_platform_unlocked("widgets"))` on `/conversations`, `/conversations/{conv_id}`, `/stats`; REQ-14 `record_widget_turn` drops the `org_id` kwarg — derived server-side via `cross_org_session` widgets-SELECT; REQ-15 `is_preview` JWT claim from `widget_preview_session`, persisted in `widget_conversations.is_preview`, all `widget_activity_stats` aggregates filter `is_preview = false`; REQ-16 `widgets.deleted_at` column + soft-delete handler (revokes kb_access + flips timestamp), partner-side lookups exclude soft-deleted, post-deploy SQL drops CASCADE on `widget_conversations.widget_id` FK; REQ-18 new `app/services/provisioning/_slug_guard.py::_assert_safe_slug` invoked at the boundary of 5 provisioning functions + `chk_portal_orgs_slug_safe` CHECK constraint on `portal_orgs.slug`. Klai-owned schema changes (REQ-15 column, REQ-16 column + CASCADE drop) live in post-deploy SQL per the `alembic-cannot-drop-non-portal_api-tables` pitfall; REQ-18 portal_api-owned slug constraint lives in `op.execute` of the alembic upgrade. |
