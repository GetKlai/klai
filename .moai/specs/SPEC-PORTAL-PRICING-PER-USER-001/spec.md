...
| 2026-05-13 | 0.5.1 | **Polish (PR #624).** Three quality-of-craft followups to v0.5.0, no behaviour change. (a) **A11y fix on the invite account-type badge.** Replaced misleading `<Label htmlFor="account-type">` (label targeting a non-focusable div) with a standalone heading `<div id="account-type-label">` + `role="status"` + `aria-live="polite"` + `aria-labelledby` on the badge — SR users hear the badge re-derivation when the Profile dropdown changes. Test pins the a11y contract. (b) **i18n key-id rename.** v0.5.0 left key ids `admin_billing_breakdown_col_seat` / `_seat_chat` / `_seat_knowledge` with values rewritten to "Account type" wording — id-vs-value drift. Renamed to `_col_account_type` / `_account_chat` / `_account_knowledge`; only consumer (`billing.lazy.tsx::seatLabel` + column header) updated in lockstep. Also rewrote the `breakdown_error` copy from "per-seat" to "per-user". (c) **File-content regex test for migration `f1ff304b7b0a`.** 11 assertions in `tests/test_pricing_per_user_drop_viewer_migration.py` pin the migration shape: chains off Phase 1 (`f7ef774cdd6b`), no UPDATE/INSERT on portal_users or portal_user_seat_history, narrows both CHECK constraints to `{chat, knowledge}`, upgrade body contains zero "viewer" string occurrences, downgrade restores three-value set. Guards the `rls-with-check-blocks-migration-update` trap class from landing silently if a future edit smuggles in a cleanup UPDATE. <br><br>Coverage delta: BE 2619 → 2630 pytest (+11); FE 225 → 238 vitest (+13 incl. a11y assertions). Status stays `shipped`. |
