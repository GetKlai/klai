...
- **Instellingen** is licht: identity-config die iedereen wel eens raakt (naam, beschrijving, leden, verwijderen). **Inzichten** is voor power-users die de KB tunen (taxonomie, sync-historie, later: chunk-coverage, RLS-debug). De Bronnen-tab is rustig.
...
| Taxonomie | Inzichten-tab → Taxonomie (kb_manager+) |
| Sync-historie, error-details | Inzichten-tab → Sync-historie |
...
- `routes/app/knowledge/$kbSlug/settings.tsx` — Instellingen-tab: Algemeen + Toegang + Verwijderen (danger zone)
- `routes/app/knowledge/$kbSlug/insights.tsx` *(nieuw)* — Inzichten-tab: Taxonomie + Sync-historie per connector
- `routes/app/knowledge/$kbSlug/taxonomy.tsx` — wordt geïmporteerd als sub-component van `insights.tsx`. URL `/taxonomy` redirect naar `/insights#taxonomie` voor backward-compat.
...
- **REQ-4** THE system SHALL exact drie tabs tonen: **Bronnen** (default), **Instellingen**, **Inzichten**. Geen extra tabs.
  - **Instellingen** = *identiteit en toegang* — light config die iedereen wel eens raakt: naam, beschrijving, leden, verwijderen. Geen power-user tools.
  - **Inzichten** = *power-user / tuning* — content-management en diagnostics voor mensen die de KB onderhouden: taxonomie (categorie-mutaties + coverage), sync-historie, later chunk-coverage / RLS-debug / top-cited.
...
- **REQ-6** THE Inzichten-tab SHALL zichtbaar zijn voor kb_manager+ en admin. Contributor en viewer zien de tab niet (taxonomie-mutaties en sync-error-details horen niet bij hun werk).
- **REQ-7** Legacy URL-querystrings (`?tab=overview|items|connectors|members|taxonomy|advanced|settings`) SHALL redirecten:
  ...
  - `taxonomy|advanced` → `/insights` (taxonomie is nu sectie binnen Inzichten)
...
- **REQ-25** THE Instellingen-tab SHALL drie secties tonen, in deze volgorde, gescheiden door `border-t border-gray-200 pt-6`:
  ...
  3. **Verwijderen** — danger zone met **"Verwijder KB"**-knop, alleen zichtbaar voor owner of admin. Hergebruikt `DeleteKbModal` (typed-name confirmation — enige toegestane modal-uitzondering per NFR-4).
- **REQ-26** Niet-owners zien sectie 1 en 2 read-only; sectie 3 is volledig verborgen. Geen taxonomie of power-user tools onder Instellingen — die horen bij Inzichten (REQ-27).

### Inzichten-tab

- **REQ-27** THE Inzichten-tab SHALL routeren naar `/insights` als URL en bestaan uit twee secties:
  1. **Taxonomie** — bestaande taxonomy-UI (categorie-tree, coverage, AI-suggestions, tag-cloud). kb_manager-gated via RoleGuard die al op `TaxonomyTab` zit.
  2. **Sync-historie** — per connector een collapsible met laatste 10 runs (status / start-time / duur / fout-reden). Data uit `connector.sync_runs`. Default ingeklapt.
- **REQ-28** Inzichten bevat actieve mutations (taxonomie wijzigen, AI-voorstellen accepteren) — dat is OK omdat de gate (kb_manager+) al filtert wie er mag tunen. Geen destructive KB-actions hier; die horen onder Instellingen (REQ-25.3).
- **REQ-29** Toekomstige power-features (RLS-debug, Qdrant-state, embedding-model-info, top-cited chunks, query-volume) horen onder Inzichten — niet onder Bronnen of Instellingen.
...
- ⏳ Drie secties: Algemeen, Toegang, Verwijderen — in deze volgorde (REQ-25)
...
- ⏳ Route `/advanced` deprecaten; redirect naar `/insights`

Inzichten-tab (rename van Geavanceerd):
- ⏳ Nieuwe route `routes/app/knowledge/$kbSlug/insights.tsx` die taxonomy-UI + sync-historie samenbrengt
- ⏳ Taxonomie geïmporteerd uit `taxonomy.tsx` als sub-component (RoleGuard blijft op kb_manager+)
- ⏳ Sync-historie sub-sectie: collapsible per connector, laatste 10 runs uit `connector.sync_runs` (Q8)
- ⏳ Tab-label "Inzichten" in `route.tsx` TAB_DEFS
- ⏳ Legacy redirects: `?tab=taxonomy|advanced` → `/insights`
- ⏳ Permissie-gate: kb_manager + admin (verbergen voor contributor + viewer)
...
| Rol | Bron toevoegen | Per-bron sync / reindex | Bron verwijderen | Sync alles | Instellingen bewerken | Inzichten zien (taxonomie + sync-historie) |
|---|---|---|---|---|---|---|
...
**Q9 — Geavanceerd hernoemen** (2026-05-11)
"Geavanceerd" hernoemd naar **Inzichten** — vriendelijkere naam, zelfde inhoud. Taxonomie + sync-historie blijven daar. Instellingen blijft licht (alleen identity-config: naam, beschrijving, leden, verwijderen). Tab-betekenis:

- Instellingen = identiteit en toegang (light, iedereen die ooit iets aanpast)
- Inzichten = power-user / tuning (taxonomie, sync-historie, toekomstige diagnostics — gated op kb_manager+)

*Iteratie note:* eerder werd geprobeerd taxonomie naar Instellingen te verhuizen om "geen mutaties op Inzichten" af te dwingen. Op reflectie verwerpt — taxonomie hoort thuis bij andere power-user surfaces, niet tussen naam-edit en delete-zone. De gate (kb_manager) doet al filterwerk.
