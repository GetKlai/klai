...
**Start:** 2026-04-19
**Laatst bijgewerkt:** 2026-04-22 (later) — post-smoke-test polish pass: AUTH-008-G flush follow-up (94025f1b), recording-cleanup 404-as-success (0ba174b0), admin prefix harmonization naar `/api/admin/*` (a452768a), FastAPI 422 validation error parsing (c7c936ec), SPEC-AUTH-004 group-product admin UI afgemaakt (08e5a25e). OBS-001 Phase C+D LIVE (f3e665a1, db8d1f68 — owned separately).
...
- [x] **AUTH-008-G** Sweep post-commit refresh op category-D RLS tables — **LIVE on main 2026-04-22** (486336a1 + ddb6cbc5 + `94025f1b` flush follow-up). 5 bestanden / 24 sites. UPDATE endpoints: refresh verwijderd. CREATE endpoints: refresh verplaatst naar vóór commit (server_default kolommen zoals `created_at`). `expert-refactoring` agent voerde de sweep uit met `RLS_DML_TABLES` als source-of-truth. Flush follow-up: `admin_widgets.py::create_widget` + `admin_api_keys.py::create_api_key` hadden geen try/flush block → refresh op PENDING object → 500. Gefixt door expliciete flush vóór refresh, geverifieerd via Playwright.
- [x] **SPEC-AUTH-004 admin UI** Group-product assignment — **LIVE on main 2026-04-22** (08e5a25e). Backend endpoints bestonden sinds SPEC-AUTH-004 maar hadden geen frontend caller. Products Card op group detail page (plan-ceiling picker + `InlineDeleteConfirm` revoke). Playwright E2E geverifieerd: assign 201 + revoke 204 + cleanup. Closes SPEC-AUTH-004 §S2.
- [x] **Polish pass (smoke-test findings)** — recording-cleanup 404-as-success (0ba174b0, TRY401 cleanup + 4 new vexa-client tests), admin route prefix harmonization `/api/admin/*` (a452768a + ruff d3a892f5), FastAPI 422 validation error parsing in `apiFetch` (c7c936ec, 4 new vitest cases).
