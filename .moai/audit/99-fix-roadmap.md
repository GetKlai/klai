...
| ~~P3~~ | ~~SEC-018~~ | Monorepo-wide Dockerfile USER audit | F-029 | 12 Dockerfiles monorepo-breed | **✅ LIVE on main** (8893a00e). Audit complete: 11 van 12 images draaien als non-root (app/klai/nextjs/mailer). caddy bewust root (privileged port binding op 80/443, accepted risk — single edge reverse-proxy, isolated op klai-net). |
| **OPEN** | SEC-020 | vexa-bot-manager + docs-app external auth audit | F-020, F-022 | extern | docs-app `lib/auth.ts` verified: JWT via JWKS met `issuer` check, **geen audience check** (valt onder SEC-012). `requireAuthOrService` accepteert ook `X-Internal-Secret` + `X-User-ID` voor service-to-service. BFF-proxy flow werkt: portal-api stuurt Bearer door, docs-app valideert tegen eigen Zitadel issuer (match). vexa-bot-manager nog niet geaudit (extern repo, aparte pass nodig). |
