...
``"klai-docker-authz"`` and the middleware unconditionally rebound it on every
...
_SERVICE_NAME = "klai-docker-authz"
...
class RequestContextMiddleware(_BaseRequestContextMiddleware):
    ...
    ``service="klai-docker-authz"`` on every request. The shared
