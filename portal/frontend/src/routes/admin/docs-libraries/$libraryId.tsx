...
export const Route = createFileRoute('/admin/docs-libraries/$libraryId')({
  component: AdminDocsLibraryDetail,
})
...
function AdminDocsLibraryDetail() {
  const auth = useAuth()
  ...
  const { data: libData, isLoading: libLoading } = useQuery({
    ...
    enabled: !!token,
    select: (data) =>
      data.docs_libraries.find((lib) => lib.id === Number(libraryId)),
  })

  // Fetch groups with access
  const { data: groupsData, isLoading: groupsLoading } = useQuery({
    queryKey: ['admin-docs-lib-groups', libraryId],
    queryFn: async () => {
      const res = await fetch(
        `${API_BASE}/api/admin/docs-libraries/${libraryId}/groups`,
        { headers: { Authorization: `Bearer ${token}` } },
      )
      if (!res.ok) throw new Error(`Failed to fetch library groups (${res.status})`)
      return res.json() as Promise<{ groups: LibGroup[] }>
    },
    enabled: !!token,
  })
  ...
  const libGroups = groupsData?.groups ?? []
  const allGroups = allGroupsData?.groups ?? []
  const grantedGroupIds = new Set(libGroups.map((g) => g.group_id))
  ...
  const grantMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSelectedGroupId('')
      toast.success(m.admin_docs_groups_grant_success())
    },
    onError: (err: Error) => {
  ...
  const revokeMutation = useMutation({
    mutationFn: async (groupId: number) => {
      const res = await fetch(
        `${API_BASE}/api/admin/docs-libraries/${libraryId}/groups/${groupId}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    onSuccess: () => {
      ...
      toast.success(m.admin_docs_groups_revoke_success())
    },
    onError: (err: Error) => {
      toast.error(err.message)
    },
    ...
    <div className="p-8 space-y-6 max-w-2xl">
      ...
      <div className="flex items-start justify-between">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate({ to: '/admin/docs-libraries' })}
        >
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between mb-1">
            <h2 className="font-semibold text-lg">{m.admin_docs_groups_title()}</h2>
          </div>
          <p className="text-sm text-[var(--color-muted-foreground)] mb-4">
            {m.admin_docs_groups_subtitle()}
          </p>
          ...
          {availableGroups.length > 0 && (
            <div className="flex gap-2 mb-4">
              ...
              >
                <option value="">{m.admin_docs_groups_add()}...</option>
                ...
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wide">
                      {m.admin_docs_groups_col_group()}
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wide">
                      {m.admin_docs_groups_col_granted()}
                    </th>
