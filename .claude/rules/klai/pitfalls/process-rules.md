...
3. Include a regression test that loads the URL with `:` in the password (`redis://:p:hPKBf@host:6379/0`) and asserts the client constructs without error. The test must NOT use a hand-crafted "valid" password — pretend the operator pasted directly from a password-manager generator.
